...
export class HomeGoToChatComponent implements OnInit {
  ...
  getCurrentProject() {
    this.auth.project_bs.pipe(takeUntil(this.unsubscribe$)).subscribe(
      (project) => {
      ...
      (error) => {
        this.logger.error(
          "[HOME-GO-TO-CHAT] $UBSCIBE TO PUBLISHED PROJECT - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "[HOME-GO-TO-CHAT] $UBSCIBE TO PUBLISHED PROJECT * COMPLETE *"
        );
      }
    );
  }
    ...
    this.projectService.getProjects().subscribe(
      (projects: any) => {
        ...
        if (projects) {
          ...
          this.logger.log(
            "[HOME-GO-TO-CHAT] getProjects this.projects ",
            projects
          );
        }

        this.current_prjct = projects.find(
      ...
      (error) => {
        this.logger.error(
          "[HOME-GO-TO-CHAT] - Find Current Project Among All: ",
          error
        );
      },
      () => {
        this.logger.log(
          "[HOME-GO-TO-CHAT] - Find Current Project Among All * COMPLETE * "
        );
      }
    );
  }
  ...
  getChatUrl() {
    ...
    this.logger.log(
      "[HOME-GO-TO-CHAT] AppConfigService getAppConfig CHAT_BASE_URL",
      this.CHAT_BASE_URL
    );
  }
