...
export class BrandService {
  ...
  async loadBrand() {
    ...
    if (environment.remoteConfig === false) {
      if (environment.hasOwnProperty("brandSrc")) {
        ...
        if (!remoteBrandUrl) {
          this.logger.log(
            "[BRAND-SERV] loadBrand remoteConfig is false - env brandSrc is empty ? ",
            remoteBrandUrl
          );
        ...
        this.logger.log(
          "[BRAND-SERV] loadBrand remoteConfig is false - env NOT has Property brandSrc -> load from assets"
        );
        this.brand = this._brand;
      }
    } else {
      ...
      if (remoteConfigData.hasOwnProperty("brandSrc")) {
        this.logger.log(
          "[BRAND-SERV] loadBrand remoteConfig is true - remoteConfigData has Property brandSrc"
        );

        const remoteBrandUrl = this.isEmpty(remoteConfigData["brandSrc"]);
        if (!remoteBrandUrl) {
          this.logger.log(
            "[BRAND-SERV] loadBrand remoteConfig is true - remoteConfigData brandSrc is empty ?",
            remoteBrandUrl
          );
        ...
        this.logger.log(
          "[BRAND-SERV] loadBrand remoteConfig is true - remoteConfigData NOT has Property brandSrc -> load from assets"
        );
  ...
  getBrand() {
    this.logger.log(
      "BrandService getBrand has been called - brand: ",
      this.brand
    );
    return {
      ...this.brand["DASHBOARD"],
      ...this.brand["COMMON"],
      ...{ WIDGET: this.brand["WIDGET"] },
    };
  }
}
