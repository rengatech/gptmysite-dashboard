...
export class DepartmentService {
  public myDepts_bs: BehaviorSubject<Department[]> = new BehaviorSubject<
    Department[]
  >([]);
  ...
  getAppConfig() {
    ...
    this.logger.log(
      "[DEPTS-SERV] getAppConfig SERVER_BASE_PATH ",
  ...
  getCurrentProjectAndBuildDeptsUrl() {
    this.auth.project_bs.subscribe((project) => {
      ...
      if (this.project) {
        this.logger.log(
          "[DEPTS-SERV] - SUBSCRIBE TO CURRENT PROJ this.project._id ",
          this.project._id
        );
        this.DEPTS_URL =
          this.SERVER_BASE_PATH + this.project._id + "/departments/";
        this.logger.log(
          "[DEPTS-SERV] - DEPTS_URL (built with SERVER_BASE_PATH) ",
          this.DEPTS_URL
        );
  ...
  public getDeptsAsOldWidget(): Observable<Department[]> {
    ...
    return this.httpClient.get<Department[]>(url, httpOptions);
  }
  ...
  public getDeptsAsNewWidget(): Observable<Department[]> {
    ...
    this.logger.log(
      "[DEPTS-SERV] - GET DEPTS AS THE NEW WIDGET VERSION URL",
      url
    );
  ...
  public getDeptsByProjectIdToHookTemplates(
    projectid
  ...
  public updateDept(
    ...
    return this.httpClient.put(url, JSON.stringify(body), httpOptions);
  }
  ...
  public updateExistingDeptWithSelectedBot(deptid: string, id_bot: string) {
    ...
    this.logger.log(
      "[DEPTS-SERV] - UPDATE EXISTING DEPT WITH SELECED BOT - URL",
      url
    );

    const body = { id_bot: id_bot };
    this.logger.log(
      "[DEPTS-SERV] - UPDATE EXISTING DEPT WITH SELECED BOT - BODY",
      body
    );

    return this.httpClient.patch(url, JSON.stringify(body), httpOptions);
  }
  ...
  public updateDefaultDeptOnlineMsg(id: string, onlineMsg: string) {
    ...
    const url = this.DEPTS_URL + id;
    this.logger.log(
      "[DEPTS-SERV] UPDATE DEFAULT DEPARTMENT ONLINE MSG URL  ",
      url
    );
    ...
    this.logger.log(
      "[DEPTS-SERV] - UPDATE DEFAULT DEPARTMENT ONLINE MSG - BODY  ",
      body
    );
    return this.httpClient.put(url, JSON.stringify(body), httpOptions);
    // .map((res) => res.json());
  ...
  public updateDefaultDeptOfflineMsg(id: string, offlineMsg: string) {
    ...
    const url = this.DEPTS_URL + id;
    this.logger.log(
      "[DEPTS-SERV] - UPDATE DEFAULT DEPARTMENT OFFLINE MSG - URL ",
      url
    );
    ...
    this.logger.log(
      "[DEPTS-SERV] - UPDATE DEFAULT DEPARTMENT OFFLINE MSG - BODY ",
      body
    );
    return this.httpClient.put(url, JSON.stringify(body), httpOptions);
  }
