...
export class WsSharedComponent implements OnInit {
  ...
  //   return index
  // }
  openChatToTheSelectedConversation(
    CHAT_BASE_URL: string,
    requestid: string,
    requester_fullanme: string
  ) {
    this.logger.log(
      "[WS-SHARED] - openChatToTheSelectedConversation - requestid",
      requestid
    );
    this.logger.log(
      "[WS-SHARED] - openChatToTheSelectedConversation - requester_fullanme",
      requester_fullanme
    );
    ...
    this.logger.log(
      "[WS-SHARED] - openChatToTheSelectedConversation - CHAT_BASE_URL",
      CHAT_BASE_URL
    );
    const chatTabCount = localStorage.getItem("tabCount");
    this.logger.log(
      "[WS-SHARED] openChatToTheSelectedConversation chatTabCount ",
      chatTabCount
    );
  ...
  createAgentsArrayFromParticipantsId(
    ...
    this.logger.log(
      "%%% WsRequestsMsgsComponent - calling createAgentsArrayFromParticipantsId"
    );
    this.agents_array = [];
    ...
    members_array.forEach((member_id) => {
      ...
      if (member_id && member_id !== "system") {
        ...
        this.logger.log(
          "%%% WsRequestsMsgsComponent - CLEANED MEMBERS ARRAY ",
          this.cleaned_members_array
        );
        this.logger.log(
          "%%% WsRequestsMsgsComponent - CLEANED MEMBERS isFirebaseUploadEngine ",
          isFirebaseUploadEngine
        );
        ...
        if (memberIsBot === true) {
          ...
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-MSGS] - CREATE-AGENT-ARRAY-FROM-PARTICIPANTS-ID - THE PARTICIP",
            member_id,
            "IS A BOT ",
            memberIsBot,
            " - ID ",
          ...
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-MSGS] - CREATE-AGENT-ARRAY-FROM-PARTICIPANTS-ID - MEMBER ",
            member_id
          );

          // l'utente è salvato nello storage
          const storeduser =
            this.usersLocalDbService.getMemberFromStorage(member_id);

          if (storeduser) {
            // console.log('[WS-SHARED][WS-REQUESTS-MSGS] - STORED USER user', storeduser)
            this.logger.log(
              "[WS-SHARED][WS-REQUESTS-MSGS] - STORED USER user id",
              storeduser["_id"]
            );
            ...
            if (member_id === storeduser["_id"]) {
              ...
              this.checkImageExists(imgUrl, (existsImage) => {
                if (existsImage == true) {
                  ...
                  this.logger.log(
                    "HERE Y2 USER storeduser  hasImage",
                    storeduser,
                    "this.agents_array ",
                    this.agents_array
                  );
                  ...
                  this.logger.log(
                    "HERE Y2 USER index (1) storeduser id",
                    storeduser["_id"]
                  );
                  this.logger.log(
                    "HERE Y2 USER index (1) member_id",
                    member_id
                  );
                  // const index = this.agents_array.findIndex(object => object.id === storeduser['_id']);
                  const index = this.agents_array.findIndex(
                    (object) => object.id === member_id
                  );
                  this.logger.log("HERE Y2 USER index (1)", index);
                  this.logger.log(
                    "HERE Y2 USER agents_array (1) ",
                    this.agents_array
                  );
                  ...
                  if (index === -1) {
                    this.agents_array.push({
                      ...
                      isBot: false,
                      hasImage: storeduser["hasImage"],
                      userfillColour: storeduser["fillColour"],
                      userFullname: storeduser["fullname_initial"],
                    });
                  }
                  ...
                  this.createAgentAvatar(storeduser);
                  this.logger.log(
                    "HERE Y2 USER index (2) storeduser id",
                    storeduser["_id"]
                  );
                  this.logger.log(
                    "HERE Y2 USER index (2) member_id",
                    member_id
                  );
                  // const index = this.agents_array.findIndex(object => object.id === storeduser['_id']);
                  const index = this.agents_array.findIndex(
            ...
            this.logger.log(
              "[WS-SHARED][WS-REQUESTS-MSGS] - there is not stored user with id ",
              member_id
            );

            this.usersService.getProjectUserById(member_id).subscribe(
              (projectuser) => {
                ...
                if (index === -1) {
                  this.agents_array.push({
                    ...
                    userfillColour: user["fillColour"],
                    userFullname: user["fullname_initial"],
                  });
                }
                this.removeDuplicateAgents(this.agents_array);
                this.usersLocalDbService.saveMembersInStorage(
                  user["_id"],
                  user,
                  "ws-shared (createAgentsArrayFromParticipantsId)"
                );
                this.logger.log(
                  "HERE Y3 USER projectuser ",
                  projectuser,
                  "this.agents_array ",
                  this.agents_array
                );
              ...
              (error) => {
                this.logger.error(
                  "[WS-SHARED][WS-REQUESTS-MSGS] - USER IS NOT IN STORAGE - GET PROJECT-USER BY ID - ERROR ",
                  error
                );
              },
              () => {
                this.logger.log(
                  "[WS-SHARED][WS-REQUESTS-MSGS] - USER IS NOT IN STORAGE - GET PROJECT-USER BY ID * COMPLETE *"
                );
                this.logger.log(
                  "[WS-SHARED][WS-REQUESTS-MSGS] this.agents_array ",
                  this.agents_array
                );
              }
            );
  ...
  doParticipatingAgentsArray(
    ...
    this.logger.log(
      "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - doParticipatingAgentsArray imageStorage ",
      imageStorage
    );
    this.logger.log(
      "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - doParticipatingAgentsArray - first_text: ",
      first_text,
      " participants: ",
      participants,
      "isFirebaseUploadEngine: ",
    ...
    participants.forEach((participantid) => {
      ...
      if (participantIsBot === true) {
        this.logger.log(
          "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - THE PARTICIP IS A BOT?",
          participantIsBot,
        ...
        if (bot) {
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - STORED BOT ",
            bot
          );
          ...
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - RUN GET BOT FROM SERVICE"
          );

          this.faqKbService.getFaqKbById(bot_id).subscribe(
            (bot: any) => {
              this.logger.log(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - GET BOT BY ID - RES",
                bot
              );
            ...
            (error) => {
              this.logger.error(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - GET BOT BY ID - ERR",
                error
              );
            },
            () => {
              this.logger.log(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - GET BOT BY ID * COMPLETE *"
              );
            }
          );
        }
        ...
        const user =
          this.usersLocalDbService.getMemberFromStorage(participantid);
        this.logger.log(
          "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - USER GET FROM STORAGE ",
          user
        );
        if (user) {
          ...
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - USER IS NOT IN STORAGE - RUN GET FROM SERVICE participantid ",
            participantid
          );
          this.usersService.getProjectUserById(participantid).subscribe(
            (projectuser) => {
              this.logger.log(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - USER IS NOT IN STORAGE GET PROJECT-USER BY ID - RES",
                projectuser
              );
              const user: any = projectuser[0].id_user;
              this.logger.log(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - USER IS NOT IN STORAGE GET PROJECT-USER BY ID - RES > user ",
                user
              );
              ...
              this.usersLocalDbService.saveMembersInStorage(
                user["_id"],
                user,
                "ws-shared (doParticipatingAgentsArray)"
              );
            },
            (error) => {
              this.logger.error(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST][WS-SHARED] - USER IS NOT IN STORAGE - GET PROJECT-USER BY ID - ERROR ",
                error
              );
            },
            () => {
              this.logger.log(
                "[WS-SHARED][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST] - USER IS NOT IN STORAGE - GET PROJECT-USER BY ID * COMPLETE *"
              );
            }
          );
        }
  ...
  currentUserIdIsInParticipants(
    participants: any,
    currentUserID: string,
    request_id
  ): boolean {
    this.logger.log(
      "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][HISTORY & NORT-CONVS][WS-REQUESTS-LIST][WS-REQUESTS-MSGS] - currentUserIdIsInParticipants participants ",
      participants,
      " currentUserID ",
      currentUserID
    );
  ...
  getConversationTypeInRequests(ws_requests) {
    ...
    ws_requests.forEach((request) => {
      ...
      if (request.channel.name === CHANNELS_NAME.VOICE_VXML) {
        channelObjct = CHANNELS.find(
          (el) => el.id === CHANNELS_NAME.VOICE_VXML
        );
      }

      const index = this.conversationTypeInRequests.findIndex(
        (e) => e.id === request.channel.name
      );
  ...
  getParticipantsInRequests(ws_requests) {
      ...
      request.participants.forEach((participant) => {
        ...
        if (participantsId.indexOf(participant) === -1) {
          ...
          if (participantIsBot === true) {
            ...
            this.logger.log(
              "[WS-SHARED][WS-REQUESTS-LIST] - getParticipantsInRequests -THE PARTICIP",
              participant,
              "IS A BOT ",
              participantIsBot,
              " - ID ",
            ...
            if (bot) {
              this.participantsInRequests.push({
                _id: participant,
              ...
              this.participantsInRequests.push({
                _id: participant,
                firstname: participant + " (bot)",
              });
              this.botAgentsIdArray.push(participant);
            }
          } else {
            const user =
              this.usersLocalDbService.getMemberFromStorage(participant);
            this.logger.log(
              "% »»» WebSocketJs WF agentsArrayBuildFromRequests user",
              user
            );
    ...
    this.logger.log(
      "[WS-SHARED][WS-REQUESTS-LIST] getParticipantsInRequests participantsId ",
      participantsId
    );
    this.logger.log(
      "[WS-SHARED][WS-REQUESTS-LIST] getParticipantsInRequests ",
      this.participantsInRequests
    );
  }

  _getProjectUserByUserId(member_id) {
    this.usersService.getProjectUserByUserId(member_id).subscribe(
      (projectUser: any) => {
        this.logger.log(
          "[WS-SHARED][WS-REQUESTS-LIST][SERVED] - GET projectUser by USER-ID ",
          projectUser
        );
        if (projectUser) {
          this.logger.log(
            "[WS-SHARED][WS-REQUESTS-LIST][SERVED] - GET projectUser id",
            projectUser
          );
          ...
          this.usersLocalDbService.saveMembersInStorage(
            ...
            projectUser.id_user,
            "ws-shared _getProjectUserByUserId"
          );
        }
      },
      (error) => {
        this.logger.error(
          "[WS-SHARED][WS-REQUESTS-LIST][SERVED] projectUser by USER-ID - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "[WS-SHARED][WS-REQUESTS-LIST][SERVED] projectUser by USER-ID * COMPLETE *"
        );
      }
    );
  }
  ...
  getDeptsAndCountOfDeptsInRequests(requests_array) {
    ...
    requests_array.forEach((request, index) => {
      ...
      if (request && request.dept) {
        ...
        depts_array.push({
          _id: request.dept._id,
          deptName: request.dept.name,
        });
  ...
  openRightSideBar(message: string) {
    ...
    this.logger.log(
      "[WS-SHARED][FAQ-TEST-COMP] - ON OPEN RIGHT SIDEBAR -> RIGHT SIDEBAR HEIGHT",
      this.train_bot_sidebar_height
    );
  }
  ...
  onJoinHandled(
    ...
    this.logger.log(
      "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][WS-REQUESTS-LIST][SERVED][UNSERVED] - JOIN PRESSED"
    );
    this.logger.log(
      "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][WS-REQUESTS-LIST][SERVED][UNSERVED] - JOIN PRESSED postmessage",
      postmessage
    );

    this.wsRequestsService.addParticipant(id_request, currentUserID).subscribe(
      (data: any) => {
        this.logger.log(
          "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][WS-REQUESTS-LIST][SERVED][UNSERVED] - addParticipant TO CHAT GROUP ",
          data
        );
      },
      (err) => {
        this.logger.error(
          "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][WS-REQUESTS-LIST][SERVED][UNSERVED] - addParticipant TO CHAT GROUP - ERROR ",
          err
        );
      },
      () => {
        this.logger.log(
          "[WS-SHARED][REQUEST-DTLS-X-PANEL][WS-REQUESTS-UNSERVED-X-PANEL][WS-REQUESTS-LIST][SERVED][UNSERVED] - addParticipant TO CHAT GROUP * COMPLETE *"
        );
  ...
  getTranslationsDisplayInAppNotification() {
    this.translate
      .get("You_are_successfully_added_to_the_chat")
      .subscribe((text: string) => {
        ...
        this.notify.showWidgetStyleUpdateNotification(
          this.you_are_successfully_added_to_the_chat,
          2,
          "done"
        );
      });
  ...
  getTranslationsAndPostMessage() {
    this.translate
      ...
      .subscribe((text: string) => {
        ...
        this.you_are_successfully_added_to_the_chat = text;
        const msg = {
          action: "display_toast_join_complete",
          text: this.you_are_successfully_added_to_the_chat,
        };
    ...
    if (participants.length > 0) {
      ...
      participants.forEach((participantid) => {
        ...
        if (participantIsBot === true) {
          ...
          if (user) {
            this.logger.log(
              "!! Ws SHARED »»»»»»» createFullParticipacipantsArray participants - user get from storage ",
              user
            );
            ...
            this.logger.log(
              "!! Ws SHARED »»»»»»» createFullParticipacipantsArray participants - user NOT IN STORAGE "
            );

            this.getProjectuserByIdAndSaveInStorage(request, participantid);

            this.usersService
              .getProjectUserById(participantid)
      ...
      this.logger.log(
        "!! Ws SHARED »»»»»»» createFullParticipacipantsArray - newParticipants Array ",
        this.newParticipants
      );
    }
  }
  ...
  getProjectuserByIdAndSaveInStorage(request, userid) {
    ...
    this.usersService.getProjectUserById(userid).subscribe(
      (projectuser) => {
        this.logger.log(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getProjectuserByIdAndSaveInStorage - RES",
          projectuser
        );
        ...
        if (projectuser) {
          ...
          if (index === -1) {
            ...
            request["test"] = this.newParticipants;
            this.logger.log(
              "!! Ws SHARED »»»»»»» createFullParticipacipantsArray   this.newParticipants  QUI SI",
              this.newParticipants
            );
          }
          ...
          this.usersLocalDbService.saveMembersInStorage(
            userid,
            this.user,
            "ws-shared getProjectuserByIdAndSaveInStorage"
          );
      ...
      (error) => {
        this.logger.error(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getProjectuserByIdAndSaveInStorage - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getProjectuserByIdAndSaveInStorage * COMPLETE *"
        );
      }
    );
  ...
  getBotFromRemoteAndSaveInStorage(bot_id: string, participantid: string) {
    this.faqKbService.getFaqKbById(bot_id).subscribe(
      (res: any) => {
        this.logger.log(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getBotFromRemoteAndSaveInStorage - RES",
          res
        );
      ...
      (error) => {
        this.logger.error(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getBotFromRemoteAndSaveInStorage - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "!! Ws SHARED »»»»»»» createFullParticipacipantsArray getBotFromRemoteAndSaveInStorage * COMPLETE *"
        );
      }
    );
  }
