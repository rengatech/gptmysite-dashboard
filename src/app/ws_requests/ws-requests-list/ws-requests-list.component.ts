...
export class WsRequestsListComponent
  extends WsSharedComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @ViewChild("testwidgetbtn", { static: false })
  private testwidgetbtnRef: ElementRef;
  ...
  ngOnDestroy() {
    ...
    this.projectUserArray.forEach((projectuser) => {
      this.wsRequestsService.unsubsToToWsAllProjectUsersOfTheProject(
  ...
  openChat() {
    ...
    localStorage.setItem(
      "last_project",
      JSON.stringify(this.current_selected_prjct)
    );
  ...
  getProjectPlan() {
    this.subscription = this.prjctPlanService.projectPlan$.subscribe(
      (projectProfileData: any) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECT-PLAN - project Profile Data",
          projectProfileData
        );
  ...
  getOSCODE() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] getAppConfig public_Key",
      this.public_Key
    );
  ...
  translateString() {
    this.translate
      .get("NewRequesterCreatedSuccessfully")
    ...
    this.translate
      .get("ThisFeatureIsAvailableWithTheProPlan")
      .subscribe((translation: any) => {
    ...
    this.translate
      .get("AvailableWithThePlan", { plan_name: PLAN_NAME.F })
      .subscribe((translation: any) => {
  ...
  getImageStorageAndThenProjectUsers() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - IMAGE STORAGE usersService BS value",
      this.imageStorage$
    );
    ...
    if (this.appConfigService.getConfig().uploadEngine === "firebase") {
      ...
      this.logger.log(
        "[WS-REQUESTS-LIST] - IMAGE STORAGE (getImageStorageAndThenProjectUsers)",
        this.storageBucket,
        "usecase firebase"
      );
  ...
  getTestSiteUrl() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] getAppConfig - TESTSITE_BASE_URL",
      this.TESTSITE_BASE_URL
    );
  }
  ...
  getChatUrl() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] getAppConfig - CHAT_BASE_URL",
      this.CHAT_BASE_URL
    );
  }
  ...
  openRightSideBar() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] »»»» OPEN RIGHT SIDEBAR ",
      this.OPEN_RIGHT_SIDEBAR
    );

    const elemMainContent = <HTMLElement>(
      document.querySelector(".main-content")
    );
    this.logger.log(
      "[WS-REQUESTS-LIST] - REQUEST-MAP - ON OPEN RIGHT SIDEBAR -> RIGHT SIDEBAR HEIGHT (MAIN-CONTENT)",
      elemMainContent.clientHeight
    );
    this.map_sidebar_height = elemMainContent.clientHeight - 100 + "px";
    this.logger.log(
      "[WS-REQUESTS-LIST] - REQUEST-MAP - ON OPEN RIGHT SIDEBAR -> RIGHT SIDEBAR HEIGHT",
      this.map_sidebar_height
    );
  }
  ...
  getProjectUserRole() {
    this.usersService.project_user_role_bs
      ...
      .subscribe((user_role) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECT-USER ROLE ",
          user_role
        );
        if (user_role) {
  ...
  getAllProjectUsers(imagestorage: string, isfirebaseuploadengine: boolean) {
    ...
    this.usersService.getProjectUsersByProjectId().subscribe(
      (_projectUsers: any) => {
        ...
        if (_projectUsers) {
          ...
          _projectUsers.forEach((projectuser) => {
            this.logger.log(
              "WS-REQUESTS-LIST - GET PROJECT-USERS forEach projectuser ",
              projectuser
            );
            let imgUrl = "";
            ...
            this.wsRequestsService.subscriptionToWsAllProjectUsersOfTheProject(
              projectuser.id_user._id
            );
      ...
      (error) => {
        this.logger.error(
          "[WS-REQUESTS-LIST] - GET PROJECT-USERS - ERROR ",
          error
        );
      },
      () => {
  ...
  listenToAllProjectUsersOfProject$(projectuser) {
    this.wsRequestsService.projectUsersOfProject$
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        (projectUser_from_ws_subscription) => {
          ...
          if (projectuser["_id"] === projectUser_from_ws_subscription["_id"]) {
            ...
            projectuser["number_assigned_requests_rt"] =
              projectUser_from_ws_subscription["number_assigned_requests"];
            projectuser["user_available_rt"] =
              projectUser_from_ws_subscription["user_available"];
            projectuser["isBusy_rt"] =
              projectUser_from_ws_subscription["isBusy"];
            projectuser["updatedAt_rt"] =
              projectUser_from_ws_subscription["updatedAt"];
            if (projectUser_from_ws_subscription["profileStatus"]) {
              projectuser["profileStatus_rt"] =
                projectUser_from_ws_subscription["profileStatus"];
            }
          }

          this.tempProjectUserArray.indexOf(projectuser) === -1
            ? this.tempProjectUserArray.push(projectuser)
            : this.logger.log(
                "PUSH PROJECT-USER IN tempProjectUserArray: This item already exists"
              );

          this.tempProjectUserArray.sort(function (a, b) {
            return a.user_available_rt - b.user_available_rt;
          });
        ...
        (error) => {
          this.logger.error(
            "[WS-REQUESTS-LIST] $UBSC TO WS PROJECT-USERS - ERROR ",
            error
          );
        },
        () => {
          this.logger.log(
            "[WS-REQUESTS-LIST] $UBSC TO WS PROJECT-USERS * COMPLETE *"
          );
        }
      );
  }
  ...
  getDeptsByProjectId(projectUserArray) {
    this.departmentService.getDeptsByProjectId().subscribe(
      ...
      (error) => {
        this.logger.error(
          "[WS-REQUESTS-LIST] (FILTERED FOR PROJECT ID) - ERROR",
          error
        );
      },
      () => {
        this.logger.log(
          "[WS-REQUESTS-LIST] (FILTERED FOR PROJECT ID) - COMPLETE"
        );
      }
    );
  }
  ...
  getGroupsByProjectId(projectUserArray) {
    ...
    this.groupService.getGroupsByProjectId().subscribe(
      (groups: any) => {
          ...
          this.groupsList.forEach((group) => {
            ...
            if (group.members.includes(this.currentUserID)) {
              ...
              group.members.forEach((member) => {
                memberOfAllGroups.indexOf(member) === -1
                  ? memberOfAllGroups.push(member)
                  : this.logger.log(
                      "PUSH MEMBER ID IN memberOfAllGroups : This item already exists"
                    );
              });

              // console.log('[WS-REQUESTS-LIST] - ARRAY OF ALL MEMBERS OF GROUPS ', memberOfAllGroups);

              this.filteredProjectUsersArray = projectUserArray.filter(
                (projectUser) =>
                  memberOfAllGroups.includes(projectUser.id_user._id)
              );
  ...
  listenToRequestsLength() {
    this.subscription = this.wsRequestsService.ws_All_RequestsLength$
      ...
      .subscribe((totalrequests: number) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - $UBS TO WS REQUEST LENGTH",
          totalrequests
        );
        ...
        if (totalrequests === 0) {
          ...
          this.showSpinner = false;
          this.logger.log(
            "[WS-REQUESTS-LIST] - $UBS TO WS REQUEST LENGTH (usecase totalrequests = 0) - SHOW_SIMULATE_REQUEST_BTN ",
            this.SHOW_SIMULATE_REQUEST_BTN
          );
          this.logger.log(
            "[WS-REQUESTS-LIST] - $UBS TO WS REQUEST LENGTH (usecase totalrequests = 0) - showSpinner ",
            this.showSpinner
          );
  ...
  getLoggedUser() {
    this.auth.user_bs.subscribe((user) => {
      ...
      if (user) {
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] GET CURRENT-USER > currentUser ID",
          this.currentUserID
        );
      }
    });
  ...
  getCurrentProject() {
    this.auth.project_bs.subscribe((project) => {
      ...
      if (project) {
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] GET CURRENT-PRJCT AND THEN GET PROJECT BY ID - CURRENT-PRJCT > projectId",
          this.projectId
        );
  ...
  findCurrentProjectAmongAll(projectId: string) {
    this.projectService.getProjects().subscribe(
      (projects: any) => {
        ...
        this.current_selected_prjct = projects.find(
          (prj) => prj.id_project.id === projectId
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECTS - current_selected_prjct ",
          this.current_selected_prjct
        );

        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECTS - projects ",
          projects
        );
      },
      (error) => {
  ...
  getProjectById(projectid) {
    this.projectService.getProjectById(projectid).subscribe(
      (project: any) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECT BY ID - RES: ",
          project
        );
        ...
        if (project && project.activeOperatingHours === true) {
          ...
          this.logger.log(
            "[WS-REQUESTS-LIST] - GET PROJECT BY ID operatingHoursSizeObj: ",
            operatingHoursSizeObj
          );
        }
      ...
      (error) => {
        // this.showSpinner = false;
        this.logger.error(
          "[WS-REQUESTS-LIST] - GET PROJECT BY ID - ERROR",
          error
        );
      },
      () => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET PROJECT BY ID - * COMPLETE *"
        );
      }
    );
  }
  ...
  goToProjectSettings_Advanced() {
    this.logger.log(
      "[WS-REQUESTS-LIST] HAS CLICKED goToProjectSettings_Advanced"
    );
    ...
    if (this.CURRENT_USER_ROLE === "owner") {
      if (
        ...
        this.router.navigate([
          "project/" + this.projectId + "/project-settings/advanced",
        ]);
      } else if (
        (this.profile_name === PLAN_NAME.C ||
          this.profile_name === PLAN_NAME.F) &&
  ...
  presentModalFeatureAvailableWithProPlanUserRoleOwner() {
    swal({
      ...
      if (upgradePlan) {
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] swal upgradePlan (else)",
          upgradePlan
        );
      }
    });
  ...
  seeIamAgentRequests(seeIamAgentReq) {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - seeIamAgentRequests - ONLY_MY_REQUESTS ",
      this.ONLY_MY_REQUESTS
    );
  ...
  goToDept(deptid) {
    this.router.navigate([
      "project/" + this.projectId + "/department/edit/" + deptid,
    ]);
  }
  ...
  hasmeInAgents(agents, wsrequest) {
    this.logger.log(
      "[WS-REQUESTS-LIST] - hasmeInAgents - AGENTS ",
      agents,
      " IN THE REQUEST ",
      wsrequest
    );
    this.logger.log(
      "[WS-REQUESTS-LIST] - hasmeInAgents - currentUserID ",
      this.currentUserID
    );
    if (agents) {
      ...
      this.logger.log(
        "[WS-REQUESTS-LIST] - hasmeInAgents Oops!!!! THERE ARE NOT AGENTS "
      );
    }
  }
  ...
  hasmeInParticipants(participants) {
    ...
    participants.forEach((participant) => {
      this.logger.log(
        "[WS-REQUESTS-LIST] - hasmeInParticipants - currentUserID",
        this.currentUserID
      );
      this.logger.log(
        "[WS-REQUESTS-LIST] - hasmeInParticipants - participant",
        participant
      );
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - hasmeInParticipants - iAmThere",
      iAmThere
    );
    return iAmThere;
  }
  ...
  addDeptObject(wsrequests) {
    this.departmentService
      .getDeptsByProjectIdToPromise()
      .then((_departments: any) => {
        ...
        wsrequests.forEach((request) => {
          if (request.department) {
            ...
            if (deptHasName) {
              ...
              this.logger.log(
                "[WS-REQUESTS-LIST] - (DEPTS_LAZY) GET DEPTS BY PROJECT-ID toPromise - REQ DEPT NOT HAS PROPERTY NAME",
                deptHasName
              );

              if (request.department.hasOwnProperty("_id")) {
                this.logger.log(
                  "[WS-REQUESTS-LIST] - (DEPTS_LAZY) GET DEPTS BY PROJECT-ID toPromise - REQ DEPT HAS PROPERTY ID ",
                  request.department._id,
                  " - RUN getDeptObj"
                );

                request["dept"] = this.getDeptObj(
                  request.department._id,
                  _departments
                );
              } else {
                this.logger.log(
                  "[WS-REQUESTS-LIST] - (DEPTS_LAZY) GET DEPTS BY PROJECT-ID toPromise - REQ DEPT NOT HAS PROPERTY ID ",
                  request.department._id,
                  " - RUN getDeptObj"
                );
  ...
  getDeptObj(departmentid: string, deparments: any) {
    this.logger.log(
      "[WS-REQUESTS-LIST] - (DEPTS_LAZY) getDeptObj departmentid",
      departmentid
    );
  ...
  onChangeDepts() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-DEPTS selectedDeptId",
      this.selectedDeptId
    );
    this.filter[0]["deptId"] = this.selectedDeptId;

    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-DEPTS - filter",
      this.filter
    );
    this.getWsRequests$();
  }
  ...
  clearDeptFilter() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - CLEAR DEPT FILTER - selectedDeptId",
      this.selectedDeptId
    );
  }
  ...
  onChangeAgent() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-AGENT - filter",
      this.filter
    );
    this.filter[1]["agentId"] = this.selectedAgentId;

    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-AGENT - selectedAgentId",
      this.selectedAgentId
    );

    if (this.selectedAgentId === 1) {
      this.logger.log(
        "[WS-REQUESTS-LIST] - ON-CHANGE-AGENT - >> HUMAN Agents Id Array",
    ...
    if (this.selectedAgentId === 2) {
      this.logger.log(
        "[WS-REQUESTS-LIST] - ON-CHANGE-AGENT - >> BOT Agents Id Array",
  ...
  clearAgentFilter() {
    ...
    this.hasFiltered = false;
    this.logger.log(
      "[WS-REQUESTS-LIST] - CLEAR AGENT FILTER - selectedAgentId",
      this.selectedAgentId
    );
  }
  ...
  onChangeConversationType() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-CONVERSAION-TYPE - filter",
      this.filter
    );
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - ON-CHANGE-AGENT - selectedAgentId",
      this.selectedConversationTypeId
    );
  ...
  clearConversationTypeFilter() {
    ...
    this.hasFiltered = false;
    this.logger.log(
      "[WS-REQUESTS-LIST] - CLEAR CONVERSAION-TYPE FILTER - selectedConversationTypeId",
      this.selectedConversationTypeId
    );
  }
  ...
  getWsRequests$() {
    ...
    this.wsRequestsService.wsRequestsList$
      ...
      .subscribe(
        (wsrequests) => {
          this.logger.log(
            "[WS-REQUESTS-LIST] - enter subscribe to  getWsRequests$",
            wsrequests
          );
          if (wsrequests) {
            this.logger.log(
              "[WS-REQUESTS-LIST] - getWsRequests > if (wsrequests) ",
              wsrequests
            );
            this.browserRefresh = browserRefresh;
            ...
            if (
              ...
              (this.browserRefresh === true &&
                this.wsRequestsService.wsRequestsList$.value.length > 0)
            ) {
              if (wsrequests.length > 0) {
                this.SHOW_SIMULATE_REQUEST_BTN = false;
                this.logger.log(
                  "[WS-REQUESTS-LIST] - getWsRequests$ (USECASE wsrequests.length > 0) - SHOW_SIMULATE_REQUEST_BTN ",
                  this.SHOW_SIMULATE_REQUEST_BTN
                );
                this.showSpinner = false;
                this.logger.log(
                  "[WS-REQUESTS-LIST] - getWsRequests$ (USECASE wsrequests.length > 0) - SHOW_SPINNER ",
                  this.showSpinner
                );
            ...
            if (this.ONLY_MY_REQUESTS === true) {
              ...
              wsrequests.forEach((wsrequest) => {
                if (wsrequest !== null && wsrequest !== undefined) {
                  this.logger.log(
                    "[WS-REQUESTS-LIST] - ONLY_MY_REQUESTS ? ",
                    this.ONLY_MY_REQUESTS
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] - ONLY_MY_REQUESTS - hasmeInAgents ",
                    this.hasmeInAgents(wsrequest.agents, wsrequest)
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] - ONLY_MY_REQUESTS - hasmeInParticipants ",
                    this.hasmeInParticipants(wsrequest.participants)
                  );

                  if (
                    this.hasmeInAgents(wsrequest.agents, wsrequest) === true ||
                    this.hasmeInParticipants(wsrequest.participants) === true
                  ) {
            ...
            if (this.hasFiltered === true) {
              this.ws_requests = this.ws_requests.filter((r) => {
                ...
                this.logger.log(
                  "[WS-REQUESTS-LIST] - filter filter[0]: ",
                  this.filter[0]
                );
                this.logger.log(
                  "[WS-REQUESTS-LIST] - filter filter[1]: ",
                  this.filter[1]
                );
                this.logger.log(
                  "[WS-REQUESTS-LIST] - filter filter[2]: ",
                  this.filter[2]
                );
                ...
                // -----------------------------------------------------------------------------------------------------------------------------------------------------------
                if (
                  this.filter[0] !== undefined &&
                  this.filter[0]["deptId"] !== null &&
                  this.filter[1]["agentId"] === null &&
                  this.filter[2]["conversationTypeId"] === null
                ) {
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 1 - filter only for department "
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 1 - filter[deptId] ",
                    this.filter[0]["deptId"]
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 1 - r[dept] ",
                    r["dept"]
                  );

                  if (
                    r["dept"] &&
                    r["dept"]["_id"] === this.filter[0]["deptId"]
                  ) {
                    return true;
                  } else {
                    return false;
                ...
                // -----------------------------------------------------------------------------------------------------------------------------------------------------------
                if (
                  this.filter[1] !== undefined &&
                  this.filter[1]["agentId"] !== null &&
                  this.filter[0]["deptId"] === null &&
                  this.filter[2]["conversationTypeId"] === null
                ) {
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 1 - filter only for participant"
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 1 - filter[agentId] ",
                    this.filter[1]["agentId"]
                  );
                  ...
                  if (this.filter[1]["agentId"] === 1) {
                    ...
                    if (
                      this.humanAgentsIdArray.some((participantid) =>
                    ...
                    if (
                      this.botAgentsIdArray.some((participantid) =>
                        r["participants"].includes(participantid)
                      )
                    ) {
                      return true;
                    ...
                    r["participants"].includes(this.filter[1]["agentId"])
                  ) {
                    return true;
                  } else {
                    return false;
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter only for conversation type "
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[conversationTypeId] ",
                    this.filter[2]["conversationTypeId"]
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - [channel][name]] ",
                    r["channel"]["name"]
                  );
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 3 - filter for dept & participant"
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[agentId] ",
                    this.filter[1]["agentId"]
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[deptId] ",
                    this.filter[0]["deptId"]
                  );
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter for dept & participant & conversationTypeId"
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[agentId] ",
                    this.filter[1]["agentId"]
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter for dept &  conversationTypeId"
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[agentId] ",
                    this.filter[1]["agentId"]
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter for participant conversationType"
                  );
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE  - filter[agentId] ",
                    this.filter[1]["agentId"]
                ...
                if (
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] FILTER USECASE 4 - * all filters has been cleared *"
                  );
                  this.hasFiltered = false;
                  return true;
                }
          ...
          this.ws_requests.forEach((request) => {
            ...
            this.logger.log(
              "[WS-REQUESTS-LIST] - request userAgent - USER-AGENT RESULT ",
              user_agent_result
            );

            const ua_browser =
              user_agent_result.browser.name +
              " " +
              user_agent_result.browser.version;

            request["ua_browser"] = ua_browser;

            const ua_os =
              user_agent_result.os.name + " " + user_agent_result.os.version;
            ...
            if (
              request.attributes &&
              request.attributes.last_abandoned_by_project_user
            ) {
              this.logger.log(
                "[WS-REQUESTS-LIST] - for the tooltip - request.attributes",
                request.attributes
              );
              this.logger.log(
                "[WS-REQUESTS-LIST] - for the tooltip - project_users",
                this.project_users
              );

              const project_user_id =
                request.attributes.last_abandoned_by_project_user;
              this.logger.log(
                "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED project_user_id",
                project_user_id
              );

              const users_found_in_storage_by_projectuserid =
                this.usersLocalDbService.getMemberFromStorage(project_user_id);
              this.logger.log(
                "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED project_users_found_in_storage",
                users_found_in_storage_by_projectuserid
              );

              if (users_found_in_storage_by_projectuserid !== null) {
                this.logger.log(
                  "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED project_users_found_in_storage 1",
                  users_found_in_storage_by_projectuserid
                );
                this.createArrayLast_abandoned_by_project_user(
                  users_found_in_storage_by_projectuserid,
                  request
                );
              } else {
                this.usersService
                  .getProjectUserByProjecUserId(project_user_id)
                  .subscribe(
                    (projectuser) => {
                      this.logger.log(
                        "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED getProjectUserById RES",
                        projectuser
                      );
                      ...
                      if (this.UPLOAD_ENGINE_IS_FIREBASE === true) {
                        ...
                        imgUrl =
                          ...
                          "%2Fimages%2Fthumbnails_200_200-photo.jpg";
                        this.logger.log(
                          "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED has image ",
                          imgUrl
                        );
                      }
                      this.checkImageExists(imgUrl, (existsImage) => {
                        if (existsImage == true) {
                          ...
                          this.createAgentAvatarInitialsAnfBckgrnd(
                            projectuser["id_user"]
                          );
                        }

                        this.usersLocalDbService.saveMembersInStorage(
                          projectuser["id_user"]._id,
                          projectuser["id_user"],
                          "ws-requests-list"
                        );
                        this.usersLocalDbService.saveUserInStorageWithProjectUserId(
                          projectuser["_id"],
                          projectuser["id_user"]
                        );
                    ...
                    (error) => {
                      // this.showSpinner = false;
                      this.logger.error(
                        "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED getProjectUserById - ERROR",
                        error
                      );
                    },
                    () => {
                      this.logger.log(
                        "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED getProjectUserById - COMPLETE"
                      );
                    }
                  );
              }
            ...
            if (
              ...
              for (const [key, value] of Object.entries(
                ...
                this.logger.log(
                  "[WS-REQUESTS-LIST] - OTHERS PROJECT-USER THAT HAVE ABANDONED key:value ",
                  `${key}: ${value}`
                );

                if (key !== request.attributes.last_abandoned_by_project_user) {
                  const other_project_users_found =
                    this.usersLocalDbService.getMemberFromStorage(key);

                  this.logger.log(
                    "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED other_project_users_found",
                    other_project_users_found
                  );
                  if (other_project_users_found !== null) {
                    this.logger.log(
                      "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED other_project_users_found 1",
                      other_project_users_found
                    );
                    this.createArrayOther_project_users_that_has_abandoned(
                      other_project_users_found
                    );
                  } else {
                    this.usersService
                      .getProjectUserByProjecUserId(key)
                      .subscribe(
                        (projectuser) => {
                          this.logger.log(
                            "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED getProjectUserById RES",
                            projectuser
                          );
                          ...
                          if (this.UPLOAD_ENGINE_IS_FIREBASE === true) {
                            ...
                            imgUrl =
                              ...
                              "%2Fimages%2Fthumbnails_200_200-photo.jpg";
                            this.logger.log(
                              "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED has image ",
                              imgUrl
                            );
                          }
                          this.checkImageExists(imgUrl, (existsImage) => {
                        ...
                        (error) => {
                          this.logger.error(
                            "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED getProjectUserById - ERROR",
                            error
                          );
                        },
                        () => {
                          this.logger.log(
                            "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED getProjectUserById - COMPLETE"
                          );

                          const _other_project_users_found =
                            this.usersLocalDbService.getMemberFromStorage(key);
                          this.logger.log(
                            "[WS-REQUESTS-LIST] - OTHER PROJECT-USER THAT HAS ABANDONED other_project_users_found 2",
                            _other_project_users_found
                          );
                  ...
                  this.logger.log(
                    "[WS-REQUESTS-LIST] - LAST PROJECT-USER THAT HAS ABANDONED other_project_users_that_has_abandoned_array",
                    this.other_project_users_that_has_abandoned_array
                  );
                  request["attributes"][
                    "other_project_users_that_has_abandoned_array"
                  ] = this.other_project_users_that_has_abandoned_array;
                }
              }
            }
            ...
            request["currentUserIsJoined"] = this.currentUserIdIsInParticipants(
              request.participants,
              this.auth.user_bs.value._id,
              request.request_id
            );
            ...
            if (request.status === 200) {
              ...
              if (!request["participanting_Agents"]) {
                this.logger.log(
                  "[WS-REQUESTS-LIST] - PARTICIPATING-AGENTS IS ",
                  request["participanting_Agents"],
                  " - RUN doParticipatingAgentsArray "
                );

                request["participanting_Agents"] =
                  this.doParticipatingAgentsArray(
                    request.participants,
                    request.first_text,
                    this.imageStorage$,
                    this.UPLOAD_ENGINE_IS_FIREBASE
                ...
                this.logger.log(
                  "[WS-REQUESTS-LIST] - PARTICIPATING-AGENTS IS DEFINED"
                );
              }
            }
            ...
            if (request.lead && request.lead.fullname) {
              request["requester_fullname_initial"] = avatarPlaceholder(
                request.lead.fullname
              );
              request["requester_fullname_fillColour"] = getColorBck(
                request.lead.fullname
              );
          ...
          this.logger.log(
            "[WS-REQUESTS-LIST] getWsRequests - served ",
            this.wsRequestsServed
          );
          this.logger.log(
            "[WS-REQUESTS-LIST] - getWsRequests - unserved ",
            this.wsRequestsUnserved
          );
          this.logger.log(
            "[WS-REQUESTS-LIST] getWsRequests - served length ",
            this.wsRequestsServed.length
          );
          this.logger.log(
            "[WS-REQUESTS-LIST] getWsRequests - unserved length ",
            this.wsRequestsUnserved.length
          );
          // .pipe(skip(1))
          ...
          const sum =
            this.wsRequestsServed.length + this.wsRequestsUnserved.length;
          this.served_unserved_sum = sum;
          this.logger.log(
            "[WS-REQUESTS-LIST] getWsRequests sum SERVED + UNSERVED",
            this.served_unserved_sum
          );
        ...
        (error) => {
          this.logger.error(
            "[WS-REQUESTS-LIST] getWsRequests$ - ERROR ",
            error
          );
        },
        () => {
  ...
  createArrayLast_abandoned_by_project_user(user, request) {
    ...
    request["attributes"]["last_abandoned_by_project_user_array"] =
      last_abandoned_by_project_user_array;
  }

  createArrayOther_project_users_that_has_abandoned(other_project_users_found) {
    this.logger.log(
      "[WS-REQUESTS-LIST] createArrayOther_project_users_that_has_abandoned other_project_users_found",
      other_project_users_found
    );
    let imgUrl = "";
    if (this.UPLOAD_ENGINE_IS_FIREBASE === true) {
  ...
  createNewInternalRequest() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] create internalRequest - internalRequest_message ",
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] create internalRequest - internalRequest_subject",
    ...
    this.internal_request_id =
      "support-group-" + this.project_id + "-" + uiid_no_dashes;
    this.logger.log(
    ...
    this.wsRequestsService
      ...
      .subscribe(
        (newticket: any) => {
          ...
          this.logger.log(
            "[WS-REQUESTS-LIST] create newTicketRequestId  ",
        ...
        () => {
          ...
          this.wsRequestsService
            .updateRequestWorkingStatus(this.newTicketRequestId, "new")
            .subscribe(
              (request) => {
                this.logger.log(
                  "[WS-REQUESTS-MSGS] - create internalRequest WORKING STATUS ",
                  request
                );
              },
              (error) => {
                this.logger.error(
                  "[WS-REQUESTS-MSGS] - create internalRequest WORKING STATUS - ERROR ",
                  error
                );
              },
              () => {
                this.logger.log(
                  "[WS-REQUESTS-MSGS] - create internalRequestT WORKING STATUS  * COMPLETE"
                );
              }
            );
        }
  ...
  getDepartments() {
    this.departmentService.getDeptsByProjectId().subscribe(
      (_departments: any) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET DEPTS RESPONSE ",
          _departments
        );
  ...
  selectedAssignee() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - SELECT ASSIGNEE HAS FOUND IN DEPTS: ",
      hasFound
    );

    if (hasFound.length === 0) {
      this.logger.log(
        "[WS-REQUESTS-LIST] - SELECT ASSIGNEE NOT HAS FOUND IN DEPTS: ",
        hasFound
      );
      this.assignee_dept_id = undefined;
  ...
  onChangeSelectedPriority(selectedPriority) {
    this.logger.log(
      "[WS-REQUESTS-LIST] onChangeSelectedPriority selectedPriority ",
      selectedPriority
    );
  ...
  selectRequester($event) {
    this.logger.log(
      "[WS-REQUESTS-LIST] - SELECT REQUESTER ID",
      this.selectedRequester
    );
    this.logger.log(
    ...
    this.logger.log(
      "[CREATE-TICKET] - SELECT REQUESTER $event requestertype ",
      $event.requestertype
    );
  ...
  openRequesterDetails() {
    ...
    if (this.selectedRequester) {
      if (this.requester_type === "agent") {
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] - hasFound go to ",
          this.requester_type,
          " details"
        );

        const url = this.router.createUrlTree([
          "project/" + this.projectId + "/user/edit",
          this.id_for_view_requeter_dtls,
        ]);
        this.logger.log("[WS-REQUESTS-LIST] - hasFound go to url", url);
        this.logger.log(
          "[WS-REQUESTS-LIST] - hasFound go to url.toString()",
          url.toString()
        );
  ...
  presentModalAddNewRequester() {
    this.logger.log(
      "[WS-REQUESTS-LIST] - open modal presentModalAddNewRequester "
    );
  ...
  createProjectUserAndThenNewLead() {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - CREATE-NEW-USER name ",
      this.new_user_name
    );
    this.logger.log(
      "[WS-REQUESTS-LIST] - CREATE-NEW-USER email ",
      this.new_user_email
    );

    this.contactsService.createNewProjectUserToGetNewLeadID().subscribe(
      (res) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-PROJECT-USER ",
          res
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-PROJECT-USER UUID ",
          res["uuid_user"]
        );
        if (res) {
          if (res["uuid_user"]) {
      ...
      (error) => {
        this.logger.error(
          "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-PROJECT-USER - ERROR: ",
          error
        );
      },
      () => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-PROJECT-USER - COMPLETE"
        );
      }
    );
  }
  ...
  createNewContact(lead_id: string, lead_name: string, lead_email: string) {
    this.contactsService
      .createNewLead(lead_id, lead_name, lead_email)
      .subscribe(
        (lead) => {
          this.logger.log(
            "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD -  RES ",
            lead
          );
          ...
          this.projectUserAndLeadsArray =
            this.projectUserAndLeadsArray.slice(0);
          this.logger.log(
            "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD projectUserAndLeadsArray after PUSH",
            this.projectUserAndLeadsArray
          );
        ...
        (error) => {
          this.logger.error(
            "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD - ERROR: ",
            error
          );
        },
        () => {
          ...
          this.notify.showWidgetStyleUpdateNotification(
            this.newRequesterCreatedSuccessfullyMsg,
            2,
            "done"
          );

          this.logger.log(
            "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD * COMPLETE *"
          );
        }
      );
  }
  ...
  onChangeNewRequesterEmail($event) {
    this.logger.log(
      "[WS-REQUESTS-LIST]- CREATE-NEW-USER - CREATE-NEW-LEAD ON CHANGE EMAIL: ",
      $event
    );
    this.new_requester_email_is_valid = this.validateEmail($event);
    this.logger.log(
      "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD ON CHANGE EMAIL - EMAIL IS VALID ",
      this.new_requester_email_is_valid
    );
  }
  ...
  validateEmail(mail) {
    ...
    this.logger.log(
      "[WS-REQUESTS-LIST] - CREATE-NEW-USER - CREATE-NEW-LEAD - validateEmail - You have entered an invalid email address! "
    );
    return false;
  }
  ...
  getProjectUsersAndContacts() {
    ...
    zip(projectUsers, leads, (_projectUsers: any, _leads: any) => ({
      _projectUsers,
      _leads,
    })).subscribe(
      (pair) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-LEADS - PROJECT USERS : ",
          pair._projectUsers
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-LEADS - LEADS RES: ",
          pair._leads
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-LEADS - LEADS: ",
          pair._leads["leads"]
        );
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-LEADS - PROJECT-USER-&-LEAD-ARRAY: ",
          this.projectUserAndLeadsArray
        );
      ...
      (error) => {
        ...
        this.logger.error(
          "[WS-REQUESTS-LIST]- GET P-USERS-&-LEADS - ERROR: ",
          error
        );
      },
      () => {
        this.loadingRequesters = false;
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-LEADS * COMPLETE *"
        );
      }
    );
  }
  ...
  getProjectUserBotsAndDepts() {
    ...
    zip(
      ...
      bots,
      depts,
      (_projectUsers: any, _bots: any, _depts: any) => ({
        _projectUsers,
        _bots,
        _depts,
      })
      ...
      (pair) => {
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS - PROJECT USERS : ",
          pair._projectUsers
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS - BOTS : ",
          pair._bots
        );
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS - DEPTS: ",
          pair._depts
        );
        ...
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS ARRAY: ",
          this.projectUserBotsAndDeptsArray
        );
      },
      (error) => {
        ...
        this.logger.error(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS - ERROR: ",
          error
        );
      },
      () => {
        this.loadingAssignee = false;
        this.logger.log(
          "[WS-REQUESTS-LIST] - GET P-USERS-&-BOTS-&-DEPTS * COMPLETE *"
        );
      }
    );
  }
