...
export class AppComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ) // private faqKbService: FaqKbService,
  {
    this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        ...
        const grecaptchaBadgeEl = <HTMLElement>(
    ...
    // ---------------------------------------------------------------------------------
    this.logger.log(
      "[APP-COMPONENT] getConfig chatEngine",
      appConfigService.getConfig().chatEngine
    );
    this.logger.log(
      "[APP-COMPONENT] getConfig uploadEngine",
      appConfigService.getConfig().uploadEngine
    );
    this.logger.log(
      "[APP-COMPONENT] getConfig pushEngine",
      appConfigService.getConfig().pushEngine
    );
    // if (appConfigService.getConfig().chatEngine && appConfigService.getConfig().chatEngine !== 'mqtt') {
    if (
      appConfigService.getConfig().uploadEngine === "firebase" ||
      appConfigService.getConfig().chatEngine === "firebase" ||
      appConfigService.getConfig().pushEngine === "firebase"
    ) {
      ...
      // ----------------------------
      this.logger.log(
        "[APP-COMPONENT] AppConfigService firebase_conf 1 ",
        appConfigService.getConfig().firebase
      );
      // firebase.initializeApp(firebaseConfig);
      if (
        !appConfigService.getConfig().firebase ||
        appConfigService.getConfig().firebase.apiKey === "CHANGEIT"
      ) {
        throw new Error(
          "firebase config is not defined. Please create your dashboard-config.json. See the Dashboard Installation Page"
        );
      }

      const firebase_conf = appConfigService.getConfig().firebase;
      this.logger.log(
        "[APP-COMPONENT] AppConfigService - APP-COMPONENT-TS firebase_conf 2",
        firebase_conf
      );
    ...
    this.logger.log(
      "[APP-COMPONENT] !!! =========== HELLO APP.COMP (constructor) ==========="
    );
    ...
    if (this.auth.user_bs && this.auth.user_bs.value) {
      this.logger.log(
        "[APP-COMPONENT] this.auth.user_bs.value._id ",
        this.auth.user_bs.value._id
      );
      const stored_preferred_lang = localStorage.getItem(
        this.auth.user_bs.value._id + "_lang"
      );
      this.logger.log(
        "[APP-COMPONENT] stored_preferred_lang",
  ...
  async loadStyle(data) {
    if (!data || !data.parameter) {
      let className = document.body.className.replace(
        new RegExp(/style-\S*/gm),
        ""
      );
    ...
    this.auth.project_bs.subscribe((project) => {
      if (project) {
        ...
        this.projectService.getProjects().subscribe((projects: any) => {
          ...
          if (projects) {
            this.current_selected_prjct_user = projects.find(
              (prj) => prj.id_project.id === project._id
            );
            this.logger.log(
              "[APP-COMPONENT] current_selected_prjct_user ",
              this.current_selected_prjct_user
            );
          }
        });
      }
  ...
  listenToSwPostMessage() {
    ...
    if (navigator && navigator.serviceWorker) {
      ...
      navigator.serviceWorker.addEventListener("message", function (event) {
        ...
        that.logger.log(
          "[APP-COMPONENT] FIREBASE-NOTIFICATION  - Received a message from service worker event : ",
          event
        );
        const count =
          +that.usersLocalDbService.getForegrondNotificationsCount();
        that.logger.log(
          "[APP-COMPONENT] FIREBASE-NOTIFICATION  - Received a message from service worker event count ",
          count
        );
  ...
  onStorageChanged(event: any) {
    ...
    if (
      event.key !== "dshbrd----foregroundcount" &&
      event.key !== "dshbrd----sound"
    ) {
      return;
    }
    const foregrondNotificationsCount =
      +this.usersLocalDbService.getForegrondNotificationsCount();
    ...
    this.wsRequestsService.publishAndStoreForegroundRequestCount(
      foregrondNotificationsCount
    );
    if (this.count === 0) {
  ...
  subscribeToStoredForegroundAndManageAppTab() {
    this.wsRequestsService.foregroundNotificationCount$.subscribe(
  ...
  listenToFCMForegroundMsgs() {
    ...
    try {
      ...
      messaging.onMessage((payload) => {
        ...
        const requester_avatar_initial =
          this.doRecipient_fullname_initial(recipient_fullname);
        const requester_avatar_bckgrnd =
        ...
        if (this.HIDE_FOREGROUND_NOTIFICATION === false) {
          this.notify.showForegroungPushNotification(
            payload.data.recipient_fullname,
            payload.data.text,
            link,
        ...
        if (elemNotification) {
          elemNotification.addEventListener("click", function handleClick() {
            ...
            localStorage.setItem(
              "last_project",
              JSON.stringify(self.current_selected_prjct_user)
            );
          });
        }
  ...
  showNotification(recipient_fullname, notificationBody, link) {
    ...
    if ((Notification as any).permission !== "granted") {
      ...
      notification.onclick = () => {
        ...
        localStorage.setItem(
          "last_project",
          JSON.stringify(this.current_selected_prjct_user)
        );
      };
    }
  }
  ...
  sendForegroundMsg() {
    ...
    this.notify.showForegroungPushNotification(
      "Milani Salame",
      "A new support request has been assigned to you: yuppt tutti",
      link,
      requester_avatar_initial,
      requester_avatar_bckgrnd
    );
  ...
  closeWSAndResetWsRequestsIfUserIsSignedOut_NoFB() {
    this.auth.user_bs.subscribe((user) => {
      this.logger.log(
        "[APP-COMPONENT] - closeWSAndResetWsRequestsIfUserIsSignedOut_NoFB ",
        user
      );

      if (user) {
        this.logger.log(
          "[APP-COMPONENT] - closeWSAndResetWsRequestsIfUserIsSignedOut_NoFB - User is signed in. ",
          user
        );
        this.userIsSignedIn = true;
      } else {
        this.logger.log(
          "[APP-COMPONENT] - closeWSAndResetWsRequestsIfUserIsSignedOut_NoFB - No user is signed in. ",
          user
        );
  ...
  closeWSAndResetWsRequestsIfUserIsSignedOut() {
    const self = this;
    this.logger.log(
      "[APP-COMPONENT] % »»» WebSocketJs WF - APP-COMPONENT - closeWSAndResetWsRequestsIfUserIsSignedOut ",
      typeof firebase.auth
    );
    firebase.auth().onAuthStateChanged(function (user) {
      if (user) {
        this.logger.log(
          "% »»» WebSocketJs WF - APP-COMPONENT - User is signed in. ",
          user
        );
        this.userIsSignedIn = true;
      } else {
        this.logger.log(
          "[APP-COMPONENT] % »»» WebSocketJs WF - APP-COMPONENT - closeWSAndResetWsRequestsIfUserIsSignedOut - No user is signed in. ",
          user
        );

        this.userIsSignedIn = false;
        this.logger.log(
          "[APP-COMPONENT] % »»» WebSocketJs WF - APP-COMPONENT - User is signed in. ",
          this.userIsSignedIn
        );
  ...
  hideWidgetAndForegroundNotificationInComponentDisplayedInChat() {
    this.subscription = this.router.events.subscribe((e) => {
      if (e instanceof NavigationEnd) {
        ...
        if (
          e.url.indexOf("/unserved-request-for-panel") !== -1 ||
          e.url.indexOf("/projects-for-panel") !== -1 ||
          e.url.indexOf("/request-for-panel") !== -1
        ) {
  ...
  hideElementsInAuthPage() {
    ...
    const elemSidebarWrapper = <HTMLElement>(
