...
export class SigninComponent implements OnInit {
  ...
  redirectIfLogged() {
    ...
    if (storedUser && !this.EXIST_STORED_ROUTE) {
      this.logger.log(
        "[SIGN-IN] - REDIRECT TO DASHBORD IF USER IS LOGGED-IN - STORED USER",
        storedUser
      );
  ...
  getWindowWidthAndHeight() {
    ...
    if (window.innerWidth < 992) {
      ...
      this.logger.log(
        "[SIGN-IN]- ACTUAL INNER WIDTH hide_left_panel ",
  ...
  onResize(event: any) {
    const elemLeftPanelSignin = <HTMLElement>(
    ...
    if (event.target.innerWidth < 992) {
      this.hide_left_panel = true;
      this.logger.log(
        "[SIGN-IN] - NEW INNER WIDTH hide_left_panel ",
        this.hide_left_panel
      );
    } else {
      this.hide_left_panel = false;
  ...
  signin() {
    ...
    console.log(" === SERVER_BASE_URL === ", this.appConfigService.getConfig());
    this.auth.signin(
      this.userForm.value["email"],
      this.userForm.value["password"],
      this.appConfigService.getConfig().SERVER_BASE_URL,
      (error, user) => {
        ...
        //   self.logger.error(
        //     "[SIGN-IN] 2. POST DATA ERROR status",
        //     error.status
        //   );

        //   if (error.status === 0) {
        //     this.display = "block";
        //     this.signin_errormsg =
