  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @ViewChild("fileInputLauncherBtnlogo", { static: false })
  fileInputLauncherBtnlogo: any;
  ...
  uploadLauncherButtonLogo(event) {
    if (this.hasOwnLauncherBtn) {
      swal({
        ...
        text:
          this.noDefaultLanguageIsSetUpMsg +
          ". " +
          this.setDefaultLangInMultilanguageSection,
        // content: el,
        icon: "warning",
        buttons: {
  ...
  deleteCustomLauncherLogo() {
    if (this.appConfigService.getConfig().uploadEngine === "firebase") {
      this.logger.log(
        "[USER-PROFILE] IMAGE deleteUserProfileImage with firebase service"
      );
      this.uploadImageService.deleteCustomLauncherLogo(this.id_project);
      this.subscibeHasDeletedCustomLauncherLogo();
    } else {
      this.logger.log(
        "[USER-PROFILE] IMAGE deleteUserProfileImage with native service"
      );
      this.uploadImageNativeService.deletePhotoProfile_Native(
        this.id_project,
        "user"
      );
    }
  ...
  subscribeToLauncherLogoUrl() {
    this.uploadImageService.hasUploadedLauncherLogo$.subscribe(
      (downoloadurl) => {
  ...
  setLauncherLogoUrl(imageStorage) {
    ...
    if (this.UPLOAD_ENGINE_IS_FIREBASE) {
      ...
      this.launcherLogoUrl =
        ...
        "/o/profiles%2F" +
        this.id_project +
        "%2Flauncher.jpg?alt=media";
      this.verifyLauncherLogoImgFireBase(
        this.launcherLogoUrl,
        (imageExists) => {
          if (imageExists === true) {
            this.hasOwnLauncherLogo = true;
    ...
    this.prjctPlanService.projectPlan$
      ...
      .subscribe(
        (projectProfileData: any) => {
          this.logger.log(
            "[WIDGET-SET-UP] - getProjectPlan project Profile Data",
            projectProfileData
          );
          if (projectProfileData) {
            ...
            this.subscription_end_date =
              projectProfileData.subscription_end_date;
            ...
            if (projectProfileData.extra3) {
              this.logger.log(
                "[WIDGET-SET-UP] projectProfileData.extra3 ",
                projectProfileData.extra3
              );
              this.appSumoProfile =
            ...
            if (projectProfileData.profile_type === "free") {
              if (projectProfileData.trial_expired === false) {
                ...
                if (this.profile_name === "free") {
                  this.prjct_profile_name_for_segment =
                    PLAN_NAME.B + " plan (trial)";
              ...
              if (this.subscription_is_active === true) {
                if (projectProfileData.profile_name === PLAN_NAME.A) {
                  ...
                  if (!this.appSumoProfile) {
                    ...
                    this.featureIsAvailable = false;
                    this.t_params = {
                      plan_name: this.appSumoProfilefeatureAvailableFromBPlan,
                    };
                  }
  ...
  manageWidgetUnbrandingVisibility(projectProfileData) {
    ...
    if (projectProfileData["customization"]) {
      this.logger.log(
        "[WIDGET-SET-UP] USECASE EXIST customization > widgetUnbranding (1)",
        projectProfileData["customization"]["widgetUnbranding"]
      );
    }

    if (
      projectProfileData["customization"] &&
      projectProfileData["customization"]["widgetUnbranding"] !== undefined
    ) {
      this.logger.log(
        "[WIDGET-SET-UP] USECASE A EXIST customization ",
        projectProfileData["customization"],
        " & widgetUnbranding",
        projectProfileData["customization"]["widgetUnbranding"]
      );
      ...
      if (projectProfileData["customization"]["widgetUnbranding"] === true) {
        ...
        this.logger.log(
          "[WIDGET-SET-UP] Widget unbranding USECASE A isVisibleWidgetUnbranding",
          this.isVisibleWidgetUnbranding
        );
      } else if (
        projectProfileData["customization"]["widgetUnbranding"] === false
      ) {
      ...
      this.logger.log(
        "[WIDGET-SET-UP] USECASE B EXIST customization ",
        projectProfileData["customization"],
        " BUT widgetUnbranding IS",
        projectProfileData["customization"]["widgetUnbranding"]
      );

      if (this.public_Key.includes("WUN")) {
        this.logger.log(
          "[WIDGET-SET-UP] Widget unbranding  USECASE B  (from FT) - EXIST WUN ",
          this.public_Key.includes("WUN")
        );

        this.isVisibleWidgetUnbranding = this.getWunValue();
        this.logger.log(
          "[WIDGET-SET-UP]  this.isVisibleWidgetUnbranding from FT ",
          this.isVisibleWidgetUnbranding
        );
        // if (key.includes("WUN")) {
        ...
        this.isVisibleWidgetUnbranding = false;
        this.logger.log(
          "[WIDGET-SET-UP] Widget unbranding  USECASE B (from FT) isVisibleWidgetUnbranding",
          this.isVisibleWidgetUnbranding
        );
      }
    } else if (projectProfileData["customization"] === undefined) {
      this.logger.log(
        "[WIDGET-SET-UP] USECASE C customization is  ",
        projectProfileData["customization"],
        "get value foem FT"
      );
      if (this.public_Key.includes("WUN")) {
  ...
  goToPricing() {
    this.logger.log(
      "[WIDGET-SET-UP] - goToPricing projectId ",
      this.id_project
    );
        ...
        if (this.prjct_profile_type === "free") {
            ...
            if (this.profile_name === "free") {
              if (this.USER_ROLE === "owner") {
                ...
                this.router.navigate([
                  "project/" + this.id_project + "/pricing",
                ]);
              } else {
          ...
          if (this.subscription_is_active) {
            if (this.profile_name === PLAN_NAME.A) {
              this.presentModalFeautureAvailableFromTier2(
                this.featureAvailableFromBPlan
              );
            } else if (this.profile_name === PLAN_NAME.D) {
              this.presentModalFeautureAvailableFromTier2(
                this.featureAvailableFromEPlan
              );
            }
  ...
  geti118nTranslations() {
    this.translate.get("Warning").subscribe((text: any) => {
      this.warning_translated = text;
    });

    this.translate
      .get("TheCustomPreChatFormIsEmptyAndWillBeDisabled")
      .subscribe((text: any) => {
    ...
    this.translate
      .get("AvailableFromThePlan", { plan_name: PLAN_NAME.B })
      .subscribe((translation: any) => {
        this.featureAvailableFromBPlan = translation;
      });

    this.translate
      .get("AvailableFromThePlan", { plan_name: PLAN_NAME.E })
      .subscribe((translation: any) => {
  ...
  listenSidebarIsOpened() {
    this.auth.settingSidebarIsOpned.subscribe((isopened) => {
      this.logger.log(
        "[WIDGET-SET-UP] SETTINGS-SIDEBAR isopened (FROM SUBSCRIPTION) ",
        isopened
      );
  ...
  getWidgetUrl() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] getAppConfig WIDGET_URL ",
      this.WIDGET_URL
    );
  }
  ...
  getProjectUserRole() {
    ...
    this.usersService.project_user_role_bs
      ...
      .subscribe((user_role) => {
        this.logger.log(
          "[NAVBAR] % »»» WebSocketJs WF +++++ ws-requests--- navbar - USER ROLE 2",
          user_role
        );
        if (user_role) {
  ...
  getAndManageAccordionInstallWidget() {
    ...
    for (i = 0; i < acc.length; i++) {
      this.logger.log(
        "[WIDGET-SET-UP] ACCORDION ARROW - INSTALL WIDGET - QUI ENTRO"
      );
      ...
      acc[i].addEventListener("click", function () {
        ...
        self.logger.log(
          "[WIDGET-SET-UP] ACCORDION ARROW - INSTALL WIDGET - ICON WRAP DIV",
          arrow_icon_div
        );
  ...
  close_panel_install_widget() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] close_panel_install_widget HAS_SELECT_INSTALL_WITH_CODE",
      this.HAS_SELECT_INSTALL_WITH_CODE
    );
  }
  ...
  installWithCode() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] installWithCode HAS_SELECT_INSTALL_WITH_CODE",
      this.HAS_SELECT_INSTALL_WITH_CODE
    );
  }
  ...
  installWithGTM() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] installWithCode HAS_SELECT_INSTALL_WITH_GTM",
      this.HAS_SELECT_INSTALL_WITH_GTM
    );
  }
  ...
  getAndManageAccordion() {
    ...
    for (i = 0; i < acc.length; i++) {
      ...
      const hasClosedFirstAccordion = this.localDbService.getFromStorage(
        `hasclosedfirstaccordion-${this.id_project}`
      );
      this.logger.log(
        "[WIDGET-SET-UP] hasClosedFirstAccordion get from storage",
        hasClosedFirstAccordion
      );
      if (
        hasClosedFirstAccordion === null ||
        hasClosedFirstAccordion === "false"
      ) {
        ...
        setTimeout(() => {
          ...
          this.logger.log(
            "[WIDGET-SET-UP] ACCORDION ARROW ICON WRAP DIV",
            arrow_icon_div
          );
      ...
      acc[i].addEventListener("click", function () {
        ...
        setTimeout(() => {
          ...
          if (firstAccordion.classList.contains("active")) {
            self.localDbService.setInStorage(
              `hasclosedfirstaccordion-${self.id_project}`,
              "false"
            );
        ...
        self.logger.log(
          "[WIDGET-SET-UP] ACCORDION click i",
          i,
          "acc[i]",
          acc[i]
        );
  ...
  getTestSiteUrl() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] getAppConfig [WIDGET-SET-UP] TESTSITE_BASE_URL",
      this.TESTSITE_BASE_URL
    );
  }

  getLabels() {
    this.widgetService.getLabels().subscribe(
      (labels: any) => {
        ...
        if (labels && Object.keys(labels).length > 0) {
          ...
          this.logger.log(
            "[WIDGET-SET-UP] - GET LABELS - RES > TRANSLATIONS ",
            this.translations
          );
          ...
          this.translations.forEach((translation) => {
            this.logger.log(
              "[WIDGET-SET-UP] - GET LABELS - RES >>> TRANSLATION ",
              translation
            );
            if (translation) {
              ...
              if (translation.default === true) {
                ...
                this.logger.log(
                  "[WIDGET-SET-UP] - GET LABELS ***** defaultLangCode (onInit) ",
                  this.defaultLangCode
                );
              } else {
                this.logger.log(
                  "[WIDGET-SET-UP] - GET LABELS ***** No default Lang *****  ",
                  translation
                );
          ...
          this.logger.log(
            "[WIDGET-SET-UP] - GET LABELS - defaultLangCode (onInit) 2",
            this.defaultLangCode
          );
          ...
          this.logger.log(
            "[WIDGET-SET-UP] - GET LABELS - Array of LANG CODE ",
            this.languages_codes
          );

          // const availableTranslations = [{ code: "it", name: "Italian" }, { code: "fr", name: "French" }, { code: "en", name: "English" }];
          const availableTranslations = this.doAvailableLanguageArray(
            this.languages_codes
          );
          this.logger.log(
            "[WIDGET-SET-UP] - GET LABELS - availableTranslations ",
          ...
          if (availableTranslations && availableTranslations.length > 0) {
            ...
            this.wd_availableTranslations = availableTranslations.sort(
              this.compare
            );
            this.logger.log(
              "[WIDGET-SET-UP] - GET LABELS -  ordered wd_availableTranslations",
              this.wd_availableTranslations
            );
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - GET LABELS - selectedLangCode ",
  ...
  translateAndDisplayModalNoDefaultLangIsSet() {
    this.translate.get("NoDefaultLanguage").subscribe(
      (text: any) => {
        ...
        this.toAddLanguagesToYourProjectMsg =
          text["toAddLanguagesToYourProject"];
        this.cancelMsg = text["Cancel"];
        this.noLanguagesAreSetUpMsg = text["NoLanguagesAreSetUp"];
        this.setDefaultLangInMultilanguageSection =
          text["SetYourDefaultLanguageInTheMultilanguageSection"];
      ...
      (error) => {
        this.logger.error(
          "[WIDGET-SET-UP] - translateNoDefaultLanguageIsSetUp - ERROR ",
          error
        );
      },
      () => {
  ...
  displayModalNoDefaultLangIsSetUp() {
    ...
    swal({
      ...
      this.logger.log(
        "[WIDGET-SET-UP] - displayModalNoDefaultLangAreSetUp value",
        value
      );
  ...
  scrollToMultilanguageSection() {
    this.multilanguageRef.nativeElement.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  ...
  getCurrentTranslation(selectedLangCode: string) {
    ...
    this.translations.forEach((translation) => {
      if (translation.lang.toLowerCase() === selectedLangCode) {
        ...
        this.logger.log(
          "[WIDGET-SET-UP] ***** selected translation: ",
        ...
        this.logger.log(
          "[WIDGET-SET-UP] ***** selected translation - newConversation: ",
        ...
        this.logger.log(
          "[WIDGET-SET-UP] ***** selected translation - WELCOME_TITLE: ",
          this.welcomeTitle,
          " - WELLCOME_MSG: ",
          this.welcomeMsg
        );
        ...
        // ---------------------------------------------------------------
        this.calloutTitle =
          this.selected_translation["CALLOUT_TITLE_PLACEHOLDER"];
        // this.calloutTitleForPreview =  this.calloutTitle.trim();
        this.checkIsEmoji(this.calloutTitle.trim());
        this.logger.log(
          "[WIDGET-SET-UP] - checkIsEmoji calloutTitleForPreview (on getCurrentTranslation)",
          this.calloutTitleForPreview
        );

        this.calloutMsg = this.selected_translation["CALLOUT_MSG_PLACEHOLDER"];
        this.logger.log(
          "[WIDGET-SET-UP] ***** selected translation CALLOUT_TITLE_PLACEHOLDER: ",
          this.calloutTitle,
          " - CALLOUT_MSG: ",
          this.calloutMsg
        );
        ...
        this.offlineMsg =
          this.selected_translation["LABEL_FIRST_MSG_NO_AGENTS"];
        this.logger.log(
          "[WIDGET-SET-UP] ***** selected translation ONLINE MSG : ",
          this.onlineMsg,
          " - OFFLINE MSG: ",
          this.offlineMsg
        );
        ...
        // @ Office closed msgs
        // ---------------------------------------------------------------
        this.officeClosedMsg =
          this.selected_translation["LABEL_FIRST_MSG_OPERATING_HOURS_CLOSED"];
        ...
        // -----------------------------------------------------------------------------------------------------------------------
        this.waitingTimeNotFoundMsg =
          this.selected_translation["WAITING_TIME_NOT_FOUND"];

        this.logger.log(
          '[WIDGET-SET-UP] - ***** ["WAITING_TIME_FOUND"] contains $reply_time ',
          this.selected_translation["WAITING_TIME_FOUND"].includes(
            "$reply_time"
          )
        );
        ...
        // }
        this.waitingTimeFoundMsg =
          this.selected_translation["WAITING_TIME_FOUND"];
        this.logger.log(
          "[WIDGET-SET-UP] - ***** selected translation waitingTimeNotFoundMsg: ",
          this.waitingTimeNotFoundMsg
        );
        this.logger.log(
          "[WIDGET-SET-UP] - ***** selected translation waitingTimeFoundMsg: ",
          this.waitingTimeFoundMsg
        );

        this.LABEL_COMPLETE_FORM =
          this.selected_translation["LABEL_COMPLETE_FORM"];
        ...
        this.LABEL_PRECHAT_USER_FULLNAME =
          this.selected_translation["LABEL_PRECHAT_USER_FULLNAME"];
        // this.logger.log('getCurrentTranslation this.LABEL_PRECHAT_USER_FULLNAME ', this.LABEL_PRECHAT_USER_FULLNAME)

        this.LABEL_PRECHAT_USER_EMAIL =
          this.selected_translation["LABEL_PRECHAT_USER_EMAIL"];
        this.LABEL_PRECHAT_USER_PHONE =
          this.selected_translation["LABEL_PRECHAT_USER_PHONE"];
        this.LABEL_PRECHAT_FIRST_MESSAGE =
          this.selected_translation["LABEL_PRECHAT_FIRST_MESSAGE"];
        this.LABEL_PRECHAT_STATIC_TERMS_PRIVACY =
          this.selected_translation["LABEL_PRECHAT_STATIC_TERMS_PRIVACY"];
        this.LABEL_PRECHAT_ACCEPT_TERMS_PRIVACY =
          this.selected_translation["LABEL_PRECHAT_ACCEPT_TERMS_PRIVACY"];
  ...
  makeDefaultLanguage(languageCode) {
    this.logger.log(
      "[WIDGET-SET-UP] - MAKE DAFAULT LANG - languageCode: ",
      languageCode
    );

    this.widgetService.setDefaultLanguage(languageCode).subscribe(
      (translation: any) => {
        this.logger.log(
          "[WIDGET-SET-UP] - MAKE DAFAULT LANG - RES ",
          translation
        );
        ...
        if (translation.default === true) {
          this.defaultLangCode = translation.lang.toLowerCase();
          this.logger.log(
            "[WIDGET-SET-UP] - MAKE DAFAULT LANG - defaultLangCode (makeDefaultLanguage) ",
            this.defaultLangCode
          );
        }
      },
      (error) => {
        this.logger.error(
          "[WIDGET-SET-UP] - MAKE DAFAULT LANG - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "[WIDGET-SET-UP] - MAKE DAFAULT LANG ***** * COMPLETE *"
        );
  ...
  welcomeTitleChange(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - WELCOME TITLE CHANGE: ",
      this.welcomeTitle
    );
  ...
  onChangeFooterBrand(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - FOOTER BRAND CHANGE: ",
      this.footerBrand
    );
  }
  ...
  onChangeOfficeClosedMsg(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - OFFICE CLOSED MSG CHANGE: ",
      this.officeClosedMsg
    );
    this.HAS_CHANGED_GREETINGS = true;
    // if (event.length === 0 || event) {  }
  ...
  onChangeReplyTimeTypeMsg(value) {
    this.logger.log(
      "[WIDGET-SET-UP] - ON CHANGE REPLY TIME TYPE MSG : ",
      value
    );
    ...
    if (value === "reply_time_dynamic_msg") {
      ...
      this.logger.log(
        "[WIDGET-SET-UP] - HAS_SELECT_DYMANIC_REPLY_TIME_MSG : ",
        this.HAS_SELECT_DYMANIC_REPLY_TIME_MSG
      );
  ...
  saveFooterBrandAndLauncherBtn() {
    const footerbrand_save_btn = <HTMLElement>(
      document.querySelector(".footerbrand-save-btn")
    );
    this.logger.log(
      "[WIDGET-SET-UP]) - footerbrand_save_btn: ",
      footerbrand_save_btn
    );
  ...
  saveReplyTime() {
    ...
    const save_replytime_btn = <HTMLElement>(
      document.querySelector(".save_replytime_btn")
    );
    this.logger.log(
      "[WIDGET-SET-UP]) - save_replytime_btn: ",
      save_replytime_btn
    );
    ...
    this.widgetObj["dynamicWaitTimeReply"] =
      this.HAS_SELECT_DYMANIC_REPLY_TIME_MSG;
    this.widgetService.updateWidgetProject(this.widgetObj);
  }

  setReplyTimePlaceholder() {
    const elInput = <HTMLElement>(
      document.querySelector(".waiting-time-found-msg-input")
    );
    this.logger.log(
      "[WIDGET-SET-UP] - setReplyTimePlaceholder INPUT ELEM: ",
      elInput
    );
  ...
  waitingTimeNotFoundMsgChange(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - WAITING TIME NOT FOUND CHANGE: ",
      this.waitingTimeFoundMsg
    );
  }
  ...
  waitingTimeFoundMsgChange(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - WAITING TIME FOUND CHANGE: ",
      this.waitingTimeFoundMsg
    );

    if (/\s$/.test(event)) {
      this.logger.log(
        "[WIDGET-SET-UP] - WAITING TIME FOUND CHANGE - string contains space at last"
      );
      this.addWhiteSpaceBefore = false;
    } else {
      this.logger.log(
        "[WIDGET-SET-UP] - WAITING TIME FOUND CHANGE - string does not contain space at last"
      );
  ...
  calloutTitleChange(event) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - CALLOUT TITLE CHANGE: ",
      this.calloutTitle
    );
    // if (event.length === 0) {   }
    this.checkIsEmoji(this.calloutTitle.trim());
  ...
  checkIsEmoji(calloutTitle) {
    ...
    while ((match = regex.exec(this.calloutTitleForPreview))) {
      const emoji = match[0];
      this.logger.log(
        `[WIDGET-SET-UP] checkIsEmoji emoji ${emoji} — code points: ${
          [emoji].length
        }`
      );

      this.logger.log(
        `[WIDGET-SET-UP] checkIsEmoji emoji: ${emoji} — position: `,
        this.calloutTitleForPreview.indexOf(emoji)
      );

      if (this.calloutTitleForPreview.indexOf(emoji) === 0) {
        let cutCalloutTitle = this.calloutTitleForPreview.replace(emoji, "");
        this.logger.log(
          `[WIDGET-SET-UP] checkIsEmoji emoji ${emoji} —  at position 0: `,
          emoji
        );

        this.logger.log(
          `[WIDGET-SET-UP] checkIsEmoji — CUTTED CalloutTitle: `,
          cutCalloutTitle
        );
  ...
  onSelectlang(selectedLang) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - GET LABELS - onSelectlang (onSelectlang) ",
      this.selectedLangCode
    );
  ...
  // ------------------------------------------------------------------------------------
  saveTranslation() {
    const save_online_offline_msgs = <HTMLElement>(
      document.querySelector(".save_online_offline_msgs")
    );
  ...
  saveLabels() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - defaultLangCode (saveLabels) ",
      this.defaultLangCode
    );
    this.logger.log(
      "[WIDGET-SET-UP] - selected_translation (saveLabels) ",
      this.selected_translation
    );
    ...
    this.widgetService
      .editLabels(
        this.selectedLangCode.toUpperCase(),
        isdefault,
        this.selected_translation
      )
      .subscribe(
        ...
        (error) => {
          this.logger.error(
            "[WIDGET-SET-UP] - saveTranslation - ERROR ",
            error
          );
        },
        () => {
          if (this.HAS_CHANGED_GREETINGS === true) {
            this.notify.showWidgetStyleUpdateNotification(
              this.updateWidgetSuccessNoticationMsg,
              2,
              "done"
            );
          }
  ...
  getEnDefaultTranslation() {
    this.widgetService.getEnDefaultLabels().subscribe((labels: any) => {
      this.logger.log(
        "[WIDGET-SET-UP] ***** GET labels DEFAULT TRANSLATION ***** - RES ",
        labels
      );
      if (labels) {
        this.logger.log(
          "[WIDGET-SET-UP] ***** GET labels DEFAULT TRANSLATION ***** - RES DATA",
          labels.data
        );

        if (labels.data.hasOwnProperty("LABEL_PRECHAT_USER_FULLNAME")) {
          this.logger.log(
            "[WIDGET-SET-UP] ***** GET labels DEFAULT TRANSLATION ***** - EXIST LABEL_PRECHAT_USER_FULLNAME"
          );
        } else {
          this.logger.log(
            "[WIDGET-SET-UP] ***** GET labels DEFAULT TRANSLATION ***** - NOT EXIST LABEL_PRECHAT_USER_FULLNAME"
          );
        }
      }
    });
  ...
  getCurrentProject() {
    this.sub = this.auth.project_bs.subscribe((project) => {
      if (project) {
        ...
        this.logger.log(
          "[WIDGET-SET-UP] - SUBSCRIBE TO CURRENT - PRJCT-ID ",
          this.id_project
        );
  ...
  getProjectById() {
    this.projectService.getProjectById(this.id_project).subscribe(
      (project: any) => {
        ...
        if (project.widget) {
          ...
          if (project.widget.calloutTimer) {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) CALLOUT-TIMER: ",
              this.calloutTimerSecondSelected,
              "IS DISABLED ",
              this.CALLOUT_IS_DISABLED
            );
          } else {
            // ------------------------------------------------------------------------
            // @ calloutTimer
            ...
            // ------------------------------------------------------------------------
            this.logger.log(
              "[WIDGET-SET-UP] - onInit WIDGET DEFINED BUT CALLOUT-TIMER IS: ",
              this.calloutTimerSecondSelected,
              " > SET DEFAULT "
            );
          ...
          if (project.widget.onPageChangeVisibilityDesktop) {
            this.desktopWidgetStatus =
              project.widget.onPageChangeVisibilityDesktop;
          }
          ...
          if (project.widget.onPageChangeVisibilityMobile) {
            this.mobileWidgetStatus =
              project.widget.onPageChangeVisibilityMobile;
          }
          ...
          if (project.widget.poweredBy) {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) POWERED-BY (footerBrand) : ",
              this.footerBrand
            );
          } else {
            // ------------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------------
            this.logger.log(
              "[WIDGET-SET-UP] - onInit WIDGET DEFINED BUT POWERED-BY IS: ",
              project.widget.poweredBy,
              " > SET DEFAULT "
            );
            // this.calloutTimerSecondSelected = -1;
          ...
          if (
            ...
            project.widget.logoChat !== this.widgetLogoURL
          ) {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) LOGO URL: ",
              project.widget.logoChat,
              " HAS HOWN LOGO ",
              this.hasOwnLogo,
              " LOGO IS ON",
          ...
          if (project.widget.preChatFormJson) {
            ...
            this.prechatFormTexareaJson = JSON.stringify(
              project.widget.preChatFormJson,
              null,
              4
            );
            this.removepreChatFormFieldsIfAlreadyUsed(
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) PRE-CHAT-FORM-JSON DEFINED typeof: ",
              typeof project.widget.preChatFormJson
            );
          } else {
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) PRE-CHAT-FORM-JSON NOT DEFINED "
            );
          ...
          if (project.widget.themeColor) {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) THEME COLOR: ",
              project.widget.themeColor,
              " IS UNDEFINED > SET DEFAULT ",
              this.primaryColor
            );
          ...
          // theme color opacity
          if (
            !project.widget.themeColorOpacity ||
            project.widget.themeColorOpacity === 100
          ) {
          ...
          if (project.widget.themeForegroundColor) {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) THEME-FOREGROUND COLOR: ",
              this.secondaryColor
            );
          } else {
            // ------------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------------
            this.secondaryColor =
              this.widgetDefaultSettings.themeForegroundColor;

            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) THEME-FOREGROUND COLOR: ",
              project.widget.themeForegroundColor,
              " IS UNDEFINED > SET DEFAULT ",
              this.secondaryColor
            );
          }
          ...
          if (project.widget.align && project.widget.align === "left") {
            ...
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) ALIGN: ",
              project.widget.align
            );
          } else {
            // -----------------------------------------------------------------------
            // @ Align
          ...
          // -----------------------------------------------------------------------
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET DEFINED) DYNAMIC REPLY TIME: ",
            project.widget.dynamicWaitTimeReply
          );
          ...
          if (project.widget.dynamicWaitTimeReply === true) {
            ...
            this.HAS_SELECT_STATIC_REPLY_TIME_MSG = false;
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) >  HAS_SELECT_DYMANIC_REPLY_TIME_MSG: ",
              this.HAS_SELECT_DYMANIC_REPLY_TIME_MSG
            );
            this.logger.log(
              "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) >  HAS_SELECT_DYMANIC_REPLY_TIME_MSG: ",
              this.HAS_SELECT_STATIC_REPLY_TIME_MSG
            );
          } else {
            this.HAS_SELECT_DYMANIC_REPLY_TIME_MSG = false;
            this.HAS_SELECT_STATIC_REPLY_TIME_MSG = true;
          ...
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) > SET DEFAULT LOGOURL: ",
            this.logoUrl,
            "HAS OWN LOGO ",
            this.hasOwnLogo,
            "LOGO IS ON ",
          ...
          this.hasOwnLauncherLogo = false;
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) > hasOwnLauncherBtn: ",
            this.hasOwnLauncherBtn
          );
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) > hasOwnLauncherLogo: ",
            this.hasOwnLauncherLogo
          );
          ...
          this.primaryColor = this.widgetDefaultSettings.themeColor;
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) > SET DEFAULT THEME COLOR: ",
            this.primaryColor
          );
          this.primaryColorRgb = this.hexToRgb(this.primaryColor);
          this.themeColorOpacity = "1";
          ...
          this.logger.log(
            "[WIDGET-SET-UP] - (onInit WIDGET UNDEFINED) > SET DEFAULT THEME-FOREGROUND COLOR: ",
            this.secondaryColor
          );
      ...
      (error) => {
        this.logger.error(
          "[WIDGET-SET-UP] - GET PROJECT BY ID - ERROR ",
          error
        );
  ...
  setPresetColorComb(primaryColor: string, secondaryColor: string) {
    this.logger.log(
      "setPresetColorComb widget_preview_selected ",
      this.widget_preview_selected
    );
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - setPresetCombOne ",
      primaryColor,
      secondaryColor
    );
  ...
  onChangePrimaryColor($event) {
    this.logger.log(
      "onChangePrimaryColor widget_preview_selected ",
      this.widget_preview_selected
    );
    if (this.widget_preview_selected === "0001") {
      this.DISPLAY_WIDGET_HOME = true;
    ...
    this.primaryColorRgb = this.hexToRgb(this.primaryColor);
    this.logger.log(
      "[WIDGET-SET-UP] - ON CHANGE PRIMARY COLOR - PRIMARY COLOR RGB ",
  ...
  onFocusChangePrimaryColor() {
    this.logger.log(
      "onFocusChangePrimaryColor widget_preview_selected ",
      this.widget_preview_selected
    );
  ...
  onOpenPrimaryColorDialog($event) {
    this.logger.log(
      "onOpenPrimaryColorDialog widget_preview_selected ",
      this.widget_preview_selected
    );
    if (this.widget_preview_selected === "0001") {
      this.DISPLAY_WIDGET_HOME = true;
  ...
  generateRgbaGradientAndBorder(primaryColor: string) {
    ...
    this.primaryColorRgba = new_col.replace(
      /\)/i,
  ...
  onChangeSecondaryColor(event) {
    this.logger.log(
      "onChangeSecondaryColor widget_preview_selected ",
      this.widget_preview_selected
    );
    if (this.widget_preview_selected === "0001") {
      this.DISPLAY_WIDGET_HOME = true;
  ...
  onOpenSecondaryColorDialog($event) {
    this.logger.log(
      "onOpenSecondaryColorDialog widget_preview_selected ",
      this.widget_preview_selected
    );
    if (this.widget_preview_selected === "0001") {
      this.DISPLAY_WIDGET_HOME = true;
  ...
  onFocusChangeSecondaryColor() {
    this.logger.log(
      "onFocusChangeSecondaryColor widget_preview_selected ",
      this.widget_preview_selected
    );
    if (this.widget_preview_selected === "0001") {
      this.DISPLAY_WIDGET_HOME = true;
    ...
    if (event.length === 0) {
      ...
      if (this.LOGO_IS_ON === true) {
        this.verifyImageURL(event, (imageExists) => {
          ...
          if (imageExists === true) {
            this.logger.log(
              "[WIDGET-SET-UP] - checkImage Image Exists: ",
              imageExists
            );
            this.hasOwnLogo = true;
  ...
  saveWidgetAppearance() {
    const appearance_save_btn_mobile = <HTMLElement>(
      document.querySelector(".appearance_save_btn_mobile")
    );
    const appearance_save_btn_desktop = <HTMLElement>(
      document.querySelector(".appearance-save-btn-desktop")
    );
    const appearance_save_btn_bottom = <HTMLElement>(
      document.querySelector(".appearance_save_btn_bottom")
    );
  ...
  changeDesktopWidgetVisibility(event) {
    ...
    if (this.desktop_widget_is_visible === false) {
      ...
      this.widgetObj["onPageChangeVisibilityDesktop"] =
        this.desktopWidgetStatus;
  ...
  toggleCallout($event) {
    ...
    if ($event.target.checked) {
      ...
      this.logger.log(
        "[WIDGET-SET-UP] CALLOUT TIMER - toggleCallout calloutTimerSecondSelected",
        this.calloutTimerSecondSelected
      );
    } else {
      // this.calloutTimerSecondSelected = -1;
  ...
  setSelectedCalloutTimer() {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] CALLOUT TIMER - TIMER SELECTED",
      this.calloutTimerSecondSelected
    );
    // this.CALLOUT_IS_DISABLED = false;
    // *** ADD PROPERTY
  ...
  saveCalloutSettings() {
    this.HAS_CHANGED_GREETINGS = false;
    const callout_settings_save_btn = <HTMLElement>(
      document.querySelector(".callout-settings-save-btn")
    );
  ...
  togglePrechatformCheckBox(event) {
    ...
    if (event.target.checked) {
      ...
      this.widgetService.updateWidgetProject(this.widgetObj);
      this.logger.log(
        "[WIDGET-SET-UP] - INCLUDE PRE CHAT FORM ",
  ...
  customizePrechatformFieldsCheckBox(event) {
    if (event.target.checked) {
      ...
      this.HAS_ACTIVATED_PRECHAT_CUSTOM_FIELDS = true;
      // *** ADD PROPERTY
      this.widgetObj["preChatFormCustomFieldsEnabled"] =
        this.preChatFormCustomFieldsEnabled;
      if (this.prechatFormTexareaJson) {
        const parsedPrechatFormTexareaJson = JSON.parse(
          this.prechatFormTexareaJson
        );
      ...
      this.logger.log(
        "[WIDGET-SET-UP] - ENABLE CUSTOMIZE PRE CHAT FORM ",
        event.target.checked
      );
    } else {
      this.preChatFormCustomFieldsEnabled = false;
  ...
  addPrechatFormField($event) {
    ...
    this.removepreChatFormFieldsIfAlreadyUsed(
      this.preChatFormFields,
      JSON.parse(this.prechatFormTexareaJson)
    );
  }
  ...
  // ------------------------------------------------------------------------------------
  removepreChatFormFieldsIfAlreadyUsed(
    preChatFormFields: any,
    prechatFormTexareaJson: any
  ) {
    ...
    for (var i = preChatFormFields.length - 1; i >= 0; i--) {
      for (var j = 0; j < prechatFormTexareaJson.length; j++) {
        if (
          preChatFormFields[i] &&
          preChatFormFields[i].name === prechatFormTexareaJson[j].name
        ) {
  ...
  savePrechatFormCustomFields() {
    const prechatform_savejson_btn = <HTMLElement>(
      document.querySelector(".prechatform-savejson-btn")
    );
    this.logger.log(
      "[WIDGET-SET-UP] - prechatform_savejson_btn: ",
      prechatform_savejson_btn
    );
    ...
    if (this.prechatFormArray.length > 0) {
      ...
      if (
        this.prechatFormTexareaJson !== "" &&
        this.isJsonString(this.prechatFormTexareaJson) === true
      ) {
        const parsedPrechatFormTexareaJson = JSON.parse(
          this.prechatFormTexareaJson
        );
        ...
        this.logger.log(
          "[WIDGET-SET-UP] - SAVE PRE-CHAT-FORM-JSON PARSED",
          parsedPrechatFormTexareaJson
        );
  ...
  toggleAutoRating(event) {
    if (event.target.checked) {
      ...
      this.widgetService.updateWidgetProject(this.widgetObj);
      this.logger.log(
        "[WIDGET-SET-UP] - IS ENABLE Auto Rating ",
        event.target.checked
      );
    } else {
      this.nativeRating = false;
      ...
      this.logger.log(
        "[WIDGET-SET-UP] - IS ENABLED Auto Rating",
        event.target.checked
      );
    }
  }
  ...
  aligmentLeftSelected(left_selected: boolean) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - LEFT ALIGNMENT SELECTED ",
      left_selected
    );
    this.hasSelectedLeftAlignment = true;
    this.hasSelectedRightAlignment = false;
  ...
  aligmentRightSelected(right_selected: boolean) {
    ...
    this.logger.log(
      "[WIDGET-SET-UP] - RIGHT ALIGNMENT SELECTED ",
      right_selected
    );
    this.hasSelectedLeftAlignment = false;
    this.hasSelectedRightAlignment = true;
  ...
  goToWidgetMultilanguage() {
    this.router.navigate([
      "project/" + this.id_project + "/widget/translations/w",
    ]);
  }
  ...
  avarageWaitingTimeCLOCK() {
    this.subscription = this.analyticsService
      .getDataAVGWaitingCLOCK()
      .subscribe(
        (res: any) => {
          if (res && res.length > 0) {
            if (res[0].waiting_time_avg) {
              if (
                res[0].waiting_time_avg !== null ||
                res[0].waiting_time_avg !== undefined
              ) {
                this.responseAVGtime = this.humanizer.humanize(
                  res[0].waiting_time_avg,
                  { round: true, language: this.lang }
                );

                this.logger.log(
                  "[WIDGET-SET-UP]  Waiting time: humanize  this.responseAVGtime",
                  this.responseAVGtime
                );
                // this.logger.log('waiting time funtion:', this.humanizeDurations(res[0].waiting_time_avg));
              } else {
                this.logger.log(
                  "[WIDGET-SET-UP]  waiting_time_avg null or undefined"
                );
              }
            }
          }
        ...
        (error) => {
          this.logger.error(
            "[WIDGET-SET-UP] - AVERAGE TIME CLOCK REQUEST - ERROR",
            error
          );
        },
        () => {
          this.logger.log(
            "[WIDGET-SET-UP]  - AVERAGE TIME CLOCK REQUEST * COMPLETE *"
          );
        }
      );
  }
  ...
  showWaitingTime() {
    ...
    this.analyticsService.getCurrentWaitingTime().subscribe((res: any) => {
      this.logger.log(
        "[WIDGET-SET-UP]  Waiting time: humanize  responseAVGtimeDario res",
        res
      );
      ...
      if (res && res.length > 0 && res[0].waiting_time_avg) {
        ...
        const responseAVGtimeDario = this.humanizer.humanize(
          res[0].waiting_time_avg,
          { round: true, language: this.lang }
        );

        this.logger.log(
          "[WIDGET-SET-UP]  Waiting time: humanize  responseAVGtimeDario",
          responseAVGtimeDario
        );
