...
export class WidgetMultilanguageComponent
  ...
  implements OnInit
{
  ...
  getRouteParams() {
    this.route.params.subscribe((params) => {
      ...
      if (params.calledby && params.calledby === "w") {
        ...
        this.logger.log(
          "[MULTILANGUAGE] - GET ROUTE PARAMS callingPage ",
          this.callingPage
        );
  ...
  listenSidebarIsOpened() {
    this.auth.settingSidebarIsOpned.subscribe((isopened) => {
      this.logger.log(
        "Multilanguage (widget-multilanguage) isopened (FROM SUBSCRIPTION) ",
        isopened
      );
  ...
  translateDelete() {
    this.translate.get("Delete").subscribe((text: string) => {
      this.deleteMsg = text;
    });
  }
  translateCancel() {
  ...
  getEnDefaultTranslation() {
    this.widgetService.getEnDefaultLabels().subscribe(
      (labels: any) => {
        this.logger.log(
          "Multilanguage (widget-multilanguage) ***** GET labels DEFAULT TRANSLATION ***** - RES",
          labels
        );
        if (labels) {
          ...
          this.logger.log(
            "Multilanguage (widget-multilanguage) ***** GET labels DEFAULT TRANSLATION ***** - engTraslationClone >  ",
            this.engTraslationClone
          );
        }
      },
      (error) => {
        this.logger.error(
          "Multilanguage - ***** GET labels DEFAULT TRANSLATION ***** - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "Multilanguage - ***** GET labels DEFAULT TRANSLATION ***** * COMPLETE *"
        );
  ...
  getLabels() {
    ...
    this.widgetService.getLabels().subscribe(
      (labels: any) => {
        this.logger.log(
          "Multilanguage (widget-multilanguage) labels ***** - RES",
          labels
        );
        this.logger.log(
          "Multilanguage (widget-multilanguage) labels ***** - RES typeof",
          typeof labels
        );
        // && labels.length > 0
        if (labels && Object.keys(labels).length > 0) {
          ...
          // this.logger.log('Multilanguage ***** GET labels ***** - RES > TRANSLATIONS ', labels[0].data[0]);
          this.logger.log(
            "Multilanguage ***** GET labels ***** - RES > TRANSLATIONS ",
            this.translations
          );
          ...
          this.translations.forEach((translation) => {
            this.logger.log(
              "Multilanguage (widget-multilanguage) ***** GET labels ***** - RES >>> TRANSLATION ",
              translation
            );
            ...
            if (translation) {
              ...
              if (translation.default === true) {
                ...
                this.logger.log(
                  "Multilanguage (widget-multilanguage) - defaultLangCode (onInit) ",
                  this.defaultLangCode
                );
                this.defaultLangName = this.getLanguageNameFromCode(
                  translation.lang.toLowerCase()
                );
                this.logger.log(
                  "Multilanguage (widget-multilanguage) ***** GET labels ***** - RES >>> TRANSLATION defaultLangName",
                  this.defaultLangName
                );
                this.logger.log(
                  "Multilanguage (widget-multilanguage) ***** GET labels ***** - RES >>> TRANSLATION defaultLangCode",
                  this.defaultLangCode
                );
                this._selectTranslationTab(
                  translation.lang.toLowerCase(),
                  this.defaultLangName
                );
              }
              this.logger.log(
                "Multilanguage (widget-multilanguage) ***** GET labels ***** - languages_codes.length",
                this.languages_codes.length
              );
          ...
          this.logger.log(
            "Multilanguage (widget-multilanguage) - defaultLangCode (onInit) 2",
            this.defaultLangCode
          );
          this.logger.log(
            "Multilanguage ***** GET labels ***** - Array of LANG CODE ",
            this.languages_codes
          );
      ...
      (error) => {
        this.logger.error(
          "Multilanguage ***** GET labels ***** - ERROR ",
          error
        );
      },
      () => {
  ...
  makeDefaultLanguage(languageCode) {
    this.logger.log(
      "Multilanguage (widget-multilanguage) - MAKE DAFAULT LANG - languageCode: ",
      languageCode
    );
    ...
    this.widgetService.setDefaultLanguage(languageCode).subscribe(
      (translation: any) => {
        this.logger.log(
          "Multilanguage (widget-multilanguage) - MAKE DAFAULT LANG - RES ",
          translation
        );
        ...
        if (translation.default === true) {
          this.defaultLangCode = translation.lang.toLowerCase();
          this.logger.log(
            "Multilanguage (widget-multilanguage) - MAKE DAFAULT LANG - defaultLangCode ",
            this.defaultLangCode
          );
      ...
      (error) => {
        this.logger.error(
          "Multilanguage (widget-multilanguage) - MAKE DAFAULT LANG - ERROR ",
          error
        );
      },
      () => {
        this.logger.log(
          "Multilanguage (widget-multilanguage) - MAKE DAFAULT LANG ***** * COMPLETE *"
        );
  ...
  _selectTranslationTab(langSelectedCode, langSelectedName) {
    this.logger.log(
      "Multilanguage (widget-multilanguage) _selectTranslationTab -- translations ",
      this.translations
    );
    this.selectedTranslationCode = langSelectedCode;
    this.logger.log(
      "Multilanguage (widget-multilanguage) _selectTranslationTab lang Selected Code ",
      this.selectedTranslationCode
    );
    this.selectedTranslationLabel = langSelectedName;
    this.logger.log(
      "Multilanguage (widget-multilanguage) _selectTranslationTab lang Selected Name ",
      this.selectedTranslationLabel
    );
    ...
    if (this.selectedTranslationCode !== "add") {
      this.translations.forEach((translation) => {
        if (translation) {
          this.logger.log(
            "Multilanguage (widget-multilanguage) _selectTranslationTab traslation (forEach) ",
            translation
          );

          if (translation.lang.toLowerCase() === this.selectedTranslationCode) {
            this.logger.log(
              "Multilanguage (widget-multilanguage) _selectTranslationTab traslation selected ",
              translation["data"]
            );
            ...
            for (let [key, value] of Object.entries(this.engTraslationClone)) {
              this.logger.log(
                `Multilanguage (widget-multilanguage) selectTranslationTab key : ${key} - value ${value}`
              );
              ...
              let entry = {
                labelName: enLabel,
                labelValue: translation["data"][key]
            ...
            this.logger.log(
              "Multilanguage ***** GET labels ***** - selected_translation ",
              this.selected_translation
            );
          }
        }
  ...
  addNewLanguage() {
    ...
    this.logger.log(
      "Multilanguage ***** ADD-NEW-LANG selectedTranslationCode",
      this.selectedTranslationCode
    );
    this.logger.log(
      "Multilanguage ***** ADD-NEW-LANG selectedTranslationLabel",
      this.selectedTranslationLabel
    );

    // cloneLabel CHE RITORNERA IN RESPONSE LA NUOVA LINGUA (l'inglese nel caso non sia una delle nostre lingue pretradotte)
    this.widgetService
      .cloneLabel(this.temp_SelectedLangCode.toUpperCase())
      .subscribe(
        (res: any) => {
          // this.logger.log('Multilanguage - addNewLanguage - CLONE LABEL RES ', res);
          this.logger.log(
            "Multilanguage - ADD-NEW-LANG (clone-label) RES ",
            res.data
          );
        ...
        (error) => {
          this.logger.error(
            "Multilanguage ADD-NEW-LANG (clone-label) - ERROR ",
            error
          );
        },
        () => {
          this.logger.log(
            "Multilanguage ADD-NEW-LANG (clone-label) * COMPLETE *"
          );

          this.showSheleton = false;
          this.notify.presentModalSuccessCheckModal(
            "AddTranslation",
            "Completed"
          );
        }
    ...
    const newLang = {
      code: this.temp_SelectedLangCode,
      name: this.temp_SelectedLangName,
    };
    this.logger.log("Multilanguage saveNewLanguage newLang objct ", newLang);

    this.availableTranslations.push(newLang);
    this.logger.log(
      "Multilanguage saveNewLanguage availableTranslations ",
  ...
  onChangeTranslation(event, labelName) {
    this.logger.log(
      "Multilanguage - onChangeTranslation event: ",
      event,
      " - labelName: ",
      labelName
    );
    this.translations.forEach((translation) => {
      if (translation) {
        this.logger.log(
          "Multilanguage -- onChangeTranslation - traslation ",
          translation
        );
        ...
        if (translation.lang.toLowerCase() === this.selectedTranslationCode) {
          ...
          var found_key = Object.keys(this.engTraslationClone).find(
            (key) => this.engTraslationClone[key] === labelName
          );
          this.logger.log(
            "Multilanguage - onChangeTranslation filter result found_key ",
            found_key
          );

          translation["data"][found_key] = event;
          this.logger.log(
            "Multilanguage - onChangeTranslation ========== translation[data] ",
            translation["data"]
          );

          this.currentTraslationClone = Object.assign({}, translation["data"]);
          this.logger.log(
            "Multilanguage - onChangeTranslation currentTraslationClone Translation",
            this.currentTraslationClone
          );
  ...
  editLang() {
    this.logger.log(
      "Multilanguage (widget-multilanguage) - currentTraslationClone ",
      this.currentTraslationClone
    );
    ...
    this.logger.log(
      "Multilanguage (widget-multilanguage) - selectedTranslationCode ",
      this.selectedTranslationCode
    );
    this.logger.log(
      "Multilanguage (widget-multilanguage)) - defaultLangCode ",
      this.defaultLangCode
    );
    ...
    this.currentTraslationClone["WELLCOME"] =
      this.currentTraslationClone["WELCOME"];
    this.widgetService
      .editLabels(
        this.selectedTranslationCode.toUpperCase(),
        isdefault,
        this.currentTraslationClone
      )
      .subscribe(
        (labels: any) => {
          this.logger.log(
            "Multilanguage (widget-multilanguage) - editLang RES ",
            labels
          );
        },
        (error) => {
          this.logger.error(
            "Multilanguage (widget-multilanguage)) - editLang - ERROR ",
            error
          );

          this.notify.showNotification(
            this.errorNoticationMsg,
            4,
            "report_problem"
          );
        },
        () => {
          ...
          this.logger.log(
            "Multilanguage (widget-multilanguage) - editLang * COMPLETE *"
          );
        }
      );
  }
  ...
  translateAndShowUpdateWidgetNotification() {
    this.translate.get("UpdateDeptGreetingsSuccessNoticationMsg").subscribe(
      ...
      (error) => {
        this.logger.error(
          "»» WIDGET SERVICE -  Update Widget Project Success NoticationMsg - ERROR ",
          error
        );
      },
      () => {
  ...
  onFocusSelectLang() {
    this.logger.log(
      "Multilanguage onFocusSelectLang translations ",
      this.translations
    );
  }
  ...
  onSelectlang(selectedLang) {
    if (selectedLang) {
      ...
      this.logger.log(
        "Multilanguage onSelectlang selected TEMP Lang Code ",
        this.temp_SelectedLangCode
      );
      this.logger.log(
        "Multilanguage onSelectlang selected TEMP Lang label ",
        this.temp_SelectedLangName
      );
    }
  }
  ...
  onClearSelectedLanguage() {
    this.disableAddBtn = true;
    this.logger.log(
      "Multilanguage onClearSelectedLanguage disableAddBtn ",
      this.disableAddBtn
    );
  }
  ...
  presentSwalModalDeleteLanguage() {
    ...
    swal({
      ...
      text: this.translate.instant("TheLanguageWillBeRemovedFromYourProject", {
        language_name: this.selectedTranslationLabel,
      }),
      icon: "warning",
  ...
  deleteLang() {
    this.logger.log(
      "Multilanguage deleteLang selected Translation Label",
      this.selectedTranslationLabel
    );
    const btn_delete_lang = <HTMLElement>(
      document.querySelector(".btn_delete_lang")
    );
    if (btn_delete_lang) {
    ...
    this.logger.log(
      "Multilanguage (widget-mtl) - deleteLang ->  availableTranslations before splice",
      this.availableTranslations
    );
    var foundIndex = this.availableTranslations.findIndex(
      (x) => x.code == this.selectedTranslationCode
    );

    this.logger.log(
      "Multilanguage (widget-mtl) - deleteLang ->  availableTranslations foundIndex",
      foundIndex
    );

    this.availableTranslations.splice(foundIndex, 1);
    this.logger.log(
      "Multilanguage (widget-mtl) - deleteLang ->  availableTranslations after splice ",
      this.availableTranslations
    );

    const elemLangTab = <HTMLElement>(
      document.querySelector(`#${this.selectedTranslationCode}_tab`)
    );
    this.logger.log(
      "Multilanguage (widget-mtl) - deleteLang -> bottom nav tab to remove",
      elemLangTab
    );
    ...
    this.widgetService
      .deleteLabels(this.selectedTranslationCode.toUpperCase())
      .subscribe(
        (labels: any) => {
          this.logger.log(
            "Multilanguage (widget-mtl) - deleteLang RES ",
            labels
          );
        },
        (error) => {
          this.logger.error(
            "Multilanguage (widget-mtl) - deleteLang - ERROR ",
            error
          );
          this.notify.showNotification(
            this.errorNoticationMsg,
        ...
        () => {
          ...
          this.logger.log(
            "Multilanguage (widget-mtl) - deleteLang * COMPLETE *"
          );
  ...
  translateTranslationDeleted() {
    this.translate
      ...
      .subscribe(
        ...
        (error) => {
          this.logger.error(
            "+ + + translateTranslationDeleted  - ERROR ",
            error
          );
        },
        () => {
